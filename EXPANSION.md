...
- **Forward half of the loom** (the X-Timeline idea, already shipped v175 — see `planning/FINISHED-FEATURES.md`): `buildUpcoming()` merges every real *future* date — quest `due`, `S.aftTestDate`, `S.milestones[].date`, `qual.expires`, and (once P3 lands) rite target dates — into a "what is coming" band, so the timeline reads past → now → owed.
...
- **The Reckoning** (season close): a structured **After-Action Review** (the X-AAR idea, already shipped v177 as the counseling-log-style `S.aarLog` — see `planning/FINISHED-FEATURES.md`: intended / happened / why / sustain / improve), stored on the season's `reckoning` field rather than a parallel array. Contextually promptable at a season boundary or off the existing streak-break detection (`streakBrokenDate` / `events.js`) and bad-AFT detection (`aftRegressionCard` / `aft.js`).
...
- **Skuld's one focus** = the X-SmartFocus recommender, already shipped v189 as `computeSmartFocus()` in `skills-core.js` — see `planning/FINISHED-FEATURES.md`: weight decay urgency × Path priority toward `S.branchGoal` and the next rite, surfacing the single highest-leverage skill *with its reason* — framed as "the debt the Norns say you owe your future self."
...
- **Strictly honest, strictly local:** no LLM, no `fetch`, no CDN, no fabrication — deterministic templates keyed by entry `kind`, with honest fallbacks ("the season was quiet — three oaths held, no rite crossed"). Keeps the project's standing "no in-app AI / stay deterministic" stance (`CLAUDE.md`'s offline hard rule). A dull true season reads as a dull true season.
...
- **The already-shipped workstreams feed it.** The stealth-assessment games (Phase T), gym-schedule-aware planning (FM-1/2), and card-game workouts (FM-3) — all live since v175–v189, see `planning/FINISHED-FEATURES.md` — all become *deeds the skald can sing and seasons can muster toward* — the Campaign gives them a narrative home. The pyramid Commons layer (complete since v167, all 16 Mythic trees) becomes the raw material the Legend and Norns reason over. None of that work is wasted; the Campaign is the frame that unifies it.
