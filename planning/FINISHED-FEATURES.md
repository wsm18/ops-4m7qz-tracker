...
### v203 — Third and final confirmed post-audit redesign: consolidated the 5 competing "what skill needs attention" systems on Dawn

**Files changed:** `src/tabs/today.js`, `src/styles/main.css`.

The v200-session audit's #3 finding: the Today/Dawn tab could show up to 3 *different* skill recommendations in the same render, computed by 3 independently-written urgency algorithms with 3 different thresholds — `getWarriorsFocus()`'s step 4 picked the *first* decaying skill in raw array order (not ranked by real urgency), and a separate "Skill of the day" Field Notes row ran its own ≤3-day-urgent-else-cycle-by-day-index logic. `computeSmartFocus()` (skills-core.js, v189) already existed as a real urgency+opportunity+path-weight ranker and was already the most deliberate of the bunch — its own code comment even explicitly named the other two as distinct, unreconciled systems.

**Fix:** made `computeSmartFocus()` the single source of truth for both remaining pickers. `getWarriorsFocus()`'s step 4 now calls it directly instead of `.filter(x=>x.q.type==="decay")[0]`, and also returns the picked skill's id (`skId`) so downstream code can detect agreement. The "Skill of the day" row's entire 3-branch urgency/cycling computation (urgency-sort, day-index modulo cycling) was deleted and replaced with the same `computeSmartFocus()` call — it now goes silent when nothing has real urgency or a real peak/effective gap to close, rather than always finding *something* to show via forced rotation. That's a deliberate, not accidental, behavior change: **Today's Hand** (the random 5-skill daily draw) already exists specifically for "give me something to practice when nothing's urgent," so a second rotating filler pick was a redundant, weaker copy of that same job — removing it matches the app's established "silent unless real support" precedent (X-Insight) rather than manufacturing a recommendation from thin urgency. Added one more small fix beyond the audit's literal ask: since both remaining pickers can now legitimately agree and show the *same* skill in two different cards on one page, the Field Notes row is suppressed when `getWarriorsFocus()` is already showing that exact skill (compares `skId`), so a real agreement reads as one clear answer instead of the same line printed twice. Removed the now-dead `.sk-focal-urgent`/`.sk-focal-urgent-row` CSS the deleted branch used.

Left the "Fading soon" digest and Today's Hand's random draw untouched, per the audit's own read: the digest is a genuinely different *list* (everything at risk, for awareness) rather than a competing single-recommendation, and Today's Hand's non-urgency-based variety draw was already confirmed intentional.

Verified via a throwaway Playwright script: seeded a skill with both real decay urgency and a peak/effective gap, stubbed `todaysPlan()` to isolate `getWarriorsFocus()`'s step-4 branch specifically, and confirmed `getWarriorsFocus()` and `computeSmartFocus()` now return the identical skill id (previously could disagree) and that the duplicate-suppression condition correctly evaluates true for that case. Confirmed via `npm run regress -- --shot` and a real screenshot of the Today tab on default (fresh/empty) seed data that nothing broke for the common case where no skill has started yet.

**This closes out all 3 FM-redesign-sized workstreams found by the v200-session audit** (Path-development math — v201; the 2 stuck-at-0 skills — v202; this Dawn consolidation — v203). One small deferred loose end remains and is noted in `planning/NEXT-SESSION-PROMPT.md`: reconciling the 4-skill "remember things" cluster's charters (`Memory span`/`Memory technique`/`Memory retention`/`Study & retention`) so they measure genuinely distinct things — not urgent, pick up if it comes up again.

Full ship checklist: `npm run build` → OK, `npm run check` → SYNTAX OK, `npm run regress` (incl. `--shot`) → `PAGEERRORS 0`, `badCount:0`, `total:12524`. No `SEED_SKILLS`/ladder content touched, `SKILL_LADDER_VER` stayed at 118. SW bumped to `operations-v203`. `npm run package` → produced `dist/operations.zip`.

### v204 — FM training-program quality pass 1: baseline-test warm-up safety protocol + a true beginner on-ramp

**Files changed:** `src/core/training.js`, `src/core/constants.js`, `src/tabs/plan.js`, `src/styles/main.css`.

Wyatt asked to make FM "the absolute best it can be" for real training help, given he described himself as "currently really bad at" physical fitness. Ran a deep training-program-quality audit (distinct from the code-correctness audits earlier this session) evaluating the FM subsystem against real evidence-based strength & conditioning and military-fitness-prep principles, not just code correctness. Found the adaptive engine, warm-up/cool-down science, and run progression are already genuinely sound — but 5 real program-quality gaps, ranked by severity. Wyatt confirmed via `AskUserQuestion` to start with the top 2 (highest injury-risk and lowest-accessibility gaps), plus confirmed adding nutrition guidance (a 3rd, later item) despite it being personal-diet-adjacent territory.

**1. Baseline-test warm-up safety protocol.** The monthly max-effort baseline test (`renderBaseline()`, `log.js`) previously said only "run a max-effort test... one all-out set each," with zero ramp-up guidance — the single highest injury-risk surface in the app, since it includes a genuinely novel loaded 3-rep-max deadlift for someone starting weak. Added `warmupProtocol`/`warmupStop` fields to `BASELINE_TEST`'s `max_deadlift` entry (`training.js`): a 5-stage ramp framed as % of the user's own best guess ("empty bar → ~50% ×5 → ~70% ×3 → ~85% ×1 → attempt your 3RM"), never a fabricated formula — same honesty discipline `computeTarget()` already applies elsewhere. Added a `warmupNote` (shorter generic "warm up first, don't test cold" reminder) to the other 6 bodyweight/timed baseline tests, which don't have a loaded ramp but still deserve *some* warning. Surfaced as a collapsible "⚠️ Before you test" `<details>` per exercise in the `bl-card` form — collapsed by default, doesn't clutter the UI for someone who already knows to warm up.

**2. A true beginner on-ramp.** Direct inspection confirmed the audit's finding: the push-up slot in every session (`s1`/`s3`/`s4`) offered only hand-release/pike/decline variants — nothing easier — and the easiest bodyweight run option was a straight 25-minute continuous run, no walk/run on-ramp. Added **"Knee push-ups"** as a real `alt` pool entry on all 3 push-up slots (`constants.js`) — reachable through the existing "🔀 swap" affordance (`exSwapHtml()`, unchanged mechanism, just a new pool member), not auto-prescribed, so a capable user never sees it forced on them. Added **"Run-walk build-up (beginner)"** as an alt on all 4 of Session 2's run-type slots — a real, standard run/walk interval on-ramp ("1 min run / 2 min walk, repeat") the user can self-progress through as it gets easier, no new state-tracking needed. Added `BEGINNER_RX` rows and `EX_HOWTO` guidance for both new exercises so `computeTarget()` gives them real starter numbers instead of vague fallback prose.

**Caught and fixed one real bug before it shipped, not after:** naming the run alt "Walk/run intervals (beginner)" would have collided with `cgFindRxRow()`'s word-overlap matcher — every word in the existing "Intervals" row also appears in that name, producing a tied 1.0 score, and ties resolve to whichever row comes first in the array (the wrong, harder row). Renamed to "Run-walk build-up (beginner)" to share zero tokens with "Intervals," verified via direct `computeTarget()` calls that both rows now resolve to their own correct, distinct starter numbers with zero cross-contamination.

Verified behaviorally: confirmed all 5 target slot pools (`sessionSlotPool()`) include the new alternates alongside the originals; confirmed `computeTarget()` resolves real, correct starter numbers for both new exercises; confirmed the pre-existing "Intervals" row still resolves correctly (no regression from the added rows); confirmed `EX_HOWTO` guidance text resolves for both; confirmed the baseline warm-up `<details>` render correctly collapsed, with the deadlift's 5-step ramp and stop-condition visible when expanded, via a real screenshot.

Full ship checklist: `npm run build` → OK, `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12524`. No `SEED_SKILLS`/ladder content touched, `SKILL_LADDER_VER` stayed at 118. SW bumped to `operations-v204`. `npm run package` → produced `dist/operations.zip`.

### v205 — FM training-program quality pass 2: macro deload detection, real AFT taper, nutrition guidance

**Files changed:** `src/core/training.js`, `src/tabs/log.js`, `src/tabs/plan.js`, `src/tabs/plan.html`, `src/core/state.js`, `src/styles/main.css`.

Closes out the remaining 3 items of the FM quality roadmap Wyatt confirmed in the v204 session (see that entry for the full audit context and the confirmed start order).

**1. Macro-level deload/overtraining detection.** Every existing recovery mechanism (`recoveryLoad()`'s 4-day per-muscle-group window, `spaceOutHardDays()`, `computeTarget()`'s per-exercise hold-on-hard-effort) operates narrowly — real overtraining often shows up as *several* exercises each individually within normal bounds, never tripping any single threshold. Added `detectOvertrainingTrend()` (`log.js`): looks at real logged struggle signals (session RPE ≥9, per-exercise effort ≥9 on 2+ exercises, or a cut-short flag) across the last 21 days of `S.workouts`, and only fires when ≥50% of at least 4 logged sessions in that window show a struggle signal — a real, sustained pattern, not one hard day. Silent below that threshold or with too few logged sessions to see a trend (same "don't compute from noise" precedent as X-Insight). Surfaced via a new `renderDeloadAdvisory()` card (`plan.js`) in the Coach Hub, positioned above the existing per-muscle-group recovery advisory since it's a different timescale/question, not a competing answer.

**2. A real AFT taper window.** Direct inspection confirmed the audit's finding: `pickAftMode()` returned `"mock"` (a full guided mock AFT — itself a hard, near-max-effort session) for the *entire* 0-14 day window before a declared test date, meaning mock-AFT frequency actually *increased* right up to test day with nothing backing off elsewhere — the opposite of standard taper practice. Restructured into two windows: **7-14 days out** still allows one full mock rehearsal (real value in practicing pacing/nerves under test conditions, with enough runway to recover after); **1-6 days out** now caps at `"practice"` (single-event, lighter) — never a full mock this close to test day. Separately, `planForDay()` now downgrades any `"hard"`-intensity session to `"moderate"` within that same 1-6 day taper window (skips already-moderate/easy/rest days and the test day itself), tagging the result `taper:true` so the reason is visible in the session label, not just a silent intensity change.

**3. Honest nutrition guidance.** The audit found a total blank — zero protein/caloric framing anywhere in an otherwise carefully evidence-based system, despite under-eating protein being one of the most common reasons a novice strength program stalls. Wyatt confirmed wanting this despite it being personal-diet-adjacent territory. Added `renderNutritionCard()` (`plan.js`), a collapsed "🍗 Nutrition basics" `<details>` in the Coach Hub (same pattern as the existing "Whole-body balance" section): a real protein target range (~0.7–1g/lb bodyweight/day, a well-established range) computed from the user's own logged `S.profile.weightLb` when available, falling back to the rule-of-thumb text (never a fabricated number) when it isn't; qualitative — not fabricated-precision — calorie framing (a full BMR/TDEE estimate would need sex/activity-level data this app doesn't collect and would read as more precise than it actually is); a sleep note; and the same "informational only — not medical advice" framing already established on the Vitals section (`profile.html`).

Verified behaviorally via throwaway Playwright scripts (deleted after use): `detectOvertrainingTrend()` across 4 scenarios (sparse data → silent, under-threshold struggle rate → silent, over-threshold → correct real signal with accurate counts, sessions outside the 21-day window correctly excluded) plus a direct DOM check confirming the card renders; the taper logic across a 10-day scan confirming taper only applies to originally-hard sessions within the 1-6 day window (not the test day itself, not already-light days, not days outside the window) and `pickAftMode()` correctly shifting from `"mock"` to `"practice"` once inside the taper window vs. allowing `"mock"` in the 7-14 day rehearsal window; the nutrition card rendering correctly both with and without a logged bodyweight.

Full ship checklist: `npm run build` → OK, `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12524`. No `SEED_SKILLS`/ladder content touched, `SKILL_LADDER_VER` stayed at 118. SW bumped to `operations-v205`. `npm run package` → produced `dist/operations.zip`.

### v206 — Quiz quality pass 1: SRS auto-feed mechanism + 4 new knowledge-board content categories

**Files changed:** `src/tabs/quizzes.js`, `quizbank.js`.

First shipped piece of the quiz-quality roadmap confirmed in the v204-session audit (see that entry for the full audit context and confirmed start order — this closes items 1-2).

**1. SRS auto-feed mechanism.** The audit's single biggest mechanism finding: `S.srsDecks` was 100% manually authored — a wrong quiz answer just stayed wrong until you happened to retake the whole quiz, with zero connection to the app's own real spaced-repetition engine. Added `feedQuizMissToSrs(quizKey, quizName, qIndex, q)` (`quizzes.js`), hooked into `answerQuiz()` on a first-attempt miss (`!correct && !QZ.retried` — retries on the same question during a run don't re-add it). Auto-creates one deck per quiz category ("Quiz misses: <name>") on first miss, deduped by `{quizKey, qIndex}` so re-missing the same question across separate attempts doesn't pile up duplicate cards. Reuses `srsGrade()`'s existing SM-2-lite scheduler as-is (`test.js`) — this only adds a new insertion point, zero new scheduling logic, matching the audit's own sketch. Card front shows the question + lettered choices; back shows the correct answer + the bank's existing sourced explanation.

**2. Four new content categories**, closing the audit's highest-impact confirmed gaps (grep-verified zero coverage before this): `ranks` (Rank Structure & Insignia — officer/WO/NCO rank order, insignia recognition, SMA), `radiocomms` (Radio Comms & Reports — phonetic alphabet, prowords, 9-line MEDEVAC line numbers, SALUTE report format), `drill` (Drill & Ceremony — facing movements, preparatory-vs-execution command structure, rank/file terminology), `aftknowledge` (AFT Standards Knowledge — event count, the 60-pt per-event minimum, 300/350 total standards, distinct from the app's own AFT score-*entry* tool since this tests knowledge *of* the standards). 40 new questions total (12/12/8/8), same `{q,a,c,e}` shape and sourced-explanation convention as all 16 existing categories, same 80% pass threshold. Kept scope to well-established, verifiable Army doctrine (rank insignia, standard prowords, the 9-line/SALUTE formats, this app's own already-established 60/300/350 AFT figures) rather than guessing at unit-specific procedure — the bank's existing top-of-file disclaimer ("verify against your advance sheet/TC/AR and cadre") already covers this content the same as the rest of the bank.

Verified behaviorally via throwaway Playwright scripts (deleted after use): confirmed a first-attempt miss creates a new SRS deck + card with correct front/back text, and that missing the same question again does NOT create a duplicate (dedupe works); confirmed all 4 new categories load with valid structure (every question has exactly 4 choices, a valid correct-index, non-empty text) — zero bad questions found; played a full new quiz (`aftknowledge`, all 8 questions answered correctly via real button-click simulation, not just calling `answerQuiz()` directly) end-to-end and confirmed correct final scoring (`passed:true, bestPct:100`); confirmed the bank now totals 20 categories.

Full ship checklist: `npm run build` → OK, `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12524`. No `SEED_SKILLS`/ladder content touched, `SKILL_LADDER_VER` stayed at 118. SW bumped to `operations-v206`. `npm run package` → produced `dist/operations.zip` (confirmed `quizbank.js`, a separate top-level file not concatenated into `index.html`, is included in the packaged zip).

### v207 — FM backlog cleared: SDC-specificity fix + AR 600-9 informational note

**Files changed:** `src/core/constants.js`, `src/tabs/plan.js`, `src/core/training.js`, `src/tabs/profile.js`.

Closes the 2 remaining FM items from the v204-session audit that were explicitly held back pending scope confirmation with Wyatt.

**SDC-specificity.** The audit found Sprint-Drag-Carry gets the least frequency/specificity of any AFT event — `s3` (Upper+Core) already had a "Farmer's carry" slot, but its bodyweight-day default was "Grip squeeze (grip trainer / towel)," real but isolated grip work with none of the loaded-locomotion demand SDC actually tests. Swapped the primary bodyweight option at that slot to "Loaded backpack carry (books/household items)" — a genuine no-equipment loaded-carry substitute (same movement pattern as the gym's Farmer's carry) — and kept Grip squeeze as an `alt` for anyone with nothing loadable on hand. This raises real SDC-transfer training frequency from ~1×/week (only the `s4` AFT circuit) to ~2×/week without adding a new slot or restructuring the week. Added a `BEGINNER_RX` row and `EX_HOWTO` guidance for the new exercise.

**AR 600-9 height/weight/body-fat standard.** Confirmed scope with Wyatt via `AskUserQuestion` first, given this maps to actual Army compliance, not just training advice — chose informational-only, no hard-coded numeric screening-weight or body-fat tables (those are age/gender-banded and get updated periodically; getting a real compliance-adjacent number wrong carries more weight than a training-target being imprecise). Also caught a placement error in the original audit's own suggestion: it said to connect this to "the Weight tab," but that tab is actually the Promise/vow ledger (a confirmed-intentional, unrelated personal-integrity feature per the v191 `FINISHED-FEATURES.md` entry) — added the note to the Profile tab instead, next to the existing BMI line where real height/weight data actually lives, explaining that AR 600-9 doesn't use BMI and pointing to cadre/current tables for anything compliance-related.
...
### v209 — Second-pass audit fixes: 9 confirmed bugs across security, performance, correctness, and honesty

**Files changed:** `src/tabs/quizzes.js`, `src/core/constants.js`, `src/tabs/profile.js`, `src/tabs/records.js`, `src/tabs/trophies.js`, `src/styles/main.css`, `src/core/migration.js`.

Wyatt asked to clear the FM/quiz backlog, then run a second, deeper audit pass on FM/Quizzes plus a genuinely comprehensive full-project audit ("absolutely everything should be looked at"). Dispatched 5 parallel research agents: second-pass FM, second-pass quizzes, a deep look at Awards/Weight/Profile (tabs that only got a lighter pass in the earlier v200-session audit), a cross-cutting technical audit (XSS safety, dead code, migration safety, performance, PWA/service-worker, TOC bridge), and a second-pass on skills/pyramid/Tree/Garden. All 5 came back — most subsystems are genuinely solid; findings split cleanly into confirmed mechanical bugs (fixed here) and larger design questions (queued, see `NEXT-SESSION-PROMPT.md`). This entry covers the confirmed-bug fixes only.

**Security — real stored-injection path.** `exportBattleBuddyReport()` (`records.js`) builds a full HTML document via `document.write()` — explicitly meant to be printed/shared with a battle buddy or cadre — interpolating name/rank/position/GPA/branch/skill-names/award-titles/counseling-log text with zero `esc()` escaping. A crafted string in a counseling summary or award title (both free text, no server-side sanitization) would execute when the report generates. Escaped every interpolated field. Same fix applied to Profile's emergency-info card (`S.name` interpolated raw into `innerHTML`), smaller blast radius but same root cause.

**Performance — a real, verified lag source.** `renderTrophies()` built a full ring-card (sigil + one DOM chip per level) for **every** leaf skill — all ~12,524 of them, including ~12,500+ that have never been touched — on every single `render()` cycle app-wide, regardless of which tab was visible. This was the one place in the codebase that skipped the same eager-render cap `skills.js`'s Side Deck already applies for the identical scale problem. Fixed to only build full cards for skills with `peakLevel>0` (the only skills that have anything to show on a "permanent record of every level ever reached" tab) and collapse the rest into a one-line "+N unstarted skills" note per Path. Verified via screenshot: Paths with real progress still show full ring-cards correctly, Paths with none stay collapsed to a lightweight note.

**Correctness — 3 real bugs in Profile's GPA/AFT/qualification tracking.** (1) Every GPA-history sort compared term strings directly (`"SP2025" > "FA2025"` alphabetically, even though Fall comes first) — added a real `termSortKey()` parser and fixed all 4 sort call sites plus `renderCommReadiness()`'s previously-unsorted "most recent GPA" lookup. (2) Found and fixed a related bug while fixing the sort: the GPA-history delete button keyed on the *sorted* display array's position but spliced `S.gpaHistory` (raw push order) at that same index — deleting a displayed row could silently delete a **different** semester's real entry. Fixed to key on the entry's real `id` instead of position. (3) `renderCommReadiness()`'s Qualifications-expired check read `q.expDate`, a field that's never actually written — the real field, set at save and read everywhere else in the app, is `q.expires` — so the expiry indicator was permanently dead, never flagging a stale CWST/BRM/CLS regardless of how expired it was. Also added a defensive sort to the AFT-total lookup (previously trusted raw push order, low real risk since AFT entries are always appended, but no reason to leave it fragile).

**Correctness — a second stale-hardcoded-count bug, same class as the v208 records.js fix.** The "Pass all 16 officer-knowledge quizzes" boss objective (`constants.js`) had `maxhp:16` hardcoded, already stale since the bank reached 20 categories in v206 — it falsely declared "all quizzes passed" 4 categories early. Fixed to self-heal `maxhp`/`name` to the live bank size every time a quiz completes, so it can never go stale again regardless of future bank growth.

**Board-readiness — answer-choice position never shuffled.** Only question *order* was shuffled; a given question always showed its choices in the same position on every replay/retake — a real risk of a cadet learning "it's the 2nd button" rather than the content, undermining the very board-readiness the v208 tracking feature is meant to measure. Fixed to shuffle display position independently each time a question renders, without changing the underlying scoring logic (`data-opt` still carries the real original answer index).

**Honesty — an uncited invented statistic.** Removed "~N lives potentially helped" from the blood-donation card (`profile.js`) — an arbitrary, uncited multiplier inconsistent with the app's own established bar elsewhere (e.g. the v207 AR 600-9 note deliberately avoided any fabricated number).

**Migration robustness.** `skHydrate()` only hydrates skills with `seeded===true`, but that flag was never backfilled onto skills from a save old enough to predate it — such a skill would silently never receive live ladder/guidance updates again. Added a one-line backfill in the existing-skill migration branch.

Verified behaviorally via a throwaway Playwright script (deleted after use) covering all of the above: the quiz-boss self-heal; `termSortKey()`'s chronological correctness across SP/SU/FA/next-year-SP; the GPA display-order/sync/delete-by-id flow with an out-of-order-backfilled semester; `renderCommReadiness()` correctly picking the chronologically-latest AFT entry (not the array-last one) and correctly flagging an expired qualification; the lives-helped stat's removal; the Trophies performance fix producing zero full cards and one note per Path under an all-unstarted seed. Also confirmed via a real screenshot that started-skill Paths still render full, correct ring-cards.

Full ship checklist: `npm run build` → OK, `npm run check` → SYNTAX OK, `npm run regress` (incl. `--shot`) → `PAGEERRORS 0`, `badCount:0`, `total:12524`. No `SEED_SKILLS`/ladder content touched, `SKILL_LADDER_VER` stayed at 118. SW bumped to `operations-v209`. `npm run package` → produced `dist/operations.zip`.

### v210 — the full v209 queue cleared: FM detraining + PT-day scheduling, quiz timed mode + content gaps, skills/Garden/Weight/Awards/Profile fixes

**Files changed:** `src/tabs/skills.js`, `src/styles/main.css`, `src/tabs/profile.js`, `src/core/constants.js`, `src/tabs/board.js`, `src/tabs/weight.js`, `src/tabs/weight.html`, `src/core/skills-core.js`, `src/tabs/awards.js`, `src/core/auto-level.js`, `src/core/skills-data.js`, `src/tabs/log.js`, `src/core/training.js`, `src/core/state.js`, `src/tabs/plan.js`, `src/tabs/plan.html`, `src/tabs/today.js` (indirectly, via `todaysPlan()`), `src/tabs/quizzes.js`, `src/tabs/quizzes.html`, `src/_shell.html`, `quizbank.js`, `src/core/migration.js`.

Wyatt asked to clear everything left in the v209 queue in one session. All 11 numbered items plus the 2 unclosed small v200-session findings are done; item #11 (Profile donations) and the Garden XP item both had a real design fork, resolved via `AskUserQuestion` before building (Garden: normalize by Path size, not reframe copy; donations: wire into a real skill, not leave alone).

**1. Skills tab — Chain view removed.** `renderSynthesisChain()`, its `.sc-toggle`/`.sc-wrap` markup, its click delegation, and its dedicated CSS block all deleted from `skills.js`/`main.css`. Confirmed via grep it was called from nowhere else and fully redundant with the always-visible Pyramid tree section, per the v209 audit's recommendation.

**2. Garden idol growth — normalized by Path size.** The flat `+15` XP grant per skill level-up (`skPass()`/`skReachLevel()`, `skills-core.js`) made the 4 smaller Paths (~783 skills each, pre-Commons-layer size) accumulate idol XP ~2x faster relative to actual `catProgressFraction()` than the 6 larger Paths (~1565 skills each). New shared `grantPathXP(cat)` scales the grant by `BASELINE × (avgPathSize / thisPathSize)`, computed live off `S.lifeSkills` so it never drifts as `SEED_SKILLS` grows. Verified live: a `roots`-Path level-up now grants 24 XP instead of 15, matching the ~1.6x size ratio exactly.

**3. Weight ↔ Integrity cross-link.** Weight tab now shows a `data-gototab="skills"` button naming the Integrity skill's current level (`weight.js`/`weight.html`); the Integrity skill's own Work-guidance panel (`skWorkGuidance()`, `skills-core.js`) now has a dedicated branch linking back to the Weight tab instead of falling through to generic "roots" guidance.

**4. Memory-skill overlap (queued since v202) — addressed without merging/renaming.** Merging or deleting live-progress skills risked real data loss and wasn't asked for, so instead: (a) fixed Memory retention's genuinely stale/self-contradicting `howTo` text (still said "self-reported... no automatic leveling yet" even though v202 wired it to auto-level from SRS ease scores) and (b) discovered while fixing it that the skill's ladder was structurally broken — 10 rungs long, but `memRetentionLevel()` (`auto-level.js`) has always capped its output at 5, and this is a 100%-`auto` skill so `skReachLevel()` unconditionally blocks any manual tap past that cap (`CLAUDE.md` hard rule #5) — meaning rungs 6–10 were **permanently unreachable**, not just badly worded. Shortened the ladder to the true 5 reachable rungs (tiers/levels/roadmap/advance/maintain all trimmed), which required a `SKILL_LADDER_VER` bump (118→119). Verified via a migration smoke test against a simulated old save with `currentLevel:8` (only possible under the old, broken 10-rung ladder) — migration's existing clamp-down-only logic (`migration.js:86-101`) correctly clamped it to 5 with peak/history preserved, no crash. Also added one clarifying sentence each to Memory technique's and Memory retention's `why` text cross-referencing the other, since the real distinction (deliberate technique practice vs. measured SRS retention quality) wasn't obvious from the near-identical names.

**5. Blood Donation — new skill, wired to Profile's donation log.** Wyatt chose to wire `S.donations` (blood donations, not financial — the "donations" naming was ambiguous going in) into a real skill rather than leave it unintegrated. New "Blood Donation" skill (`skills-data.js`, `cat:"roots"`, `parent:"Character"`, deliberately **not** given pyramid `setKey`/`synthesizedFrom` fields since it isn't part of a real Commons synthesis chain — a fake one would've created an orphaned pyramid reference). Auto-levels via a new `donationLevel()` ladder in `auto-level.js` mirroring the real blood-bank gallon-donor milestones (1 gallon = 8 donations) — an externally verifiable count, not an invented curve. Profile's blood-donation card now links to the skill's live level.

**6. Awards — Academic Honors and ROTC Record (positions/competitions/camp results) are now editable**, not just add/delete. Followed the exact `awEdit`/`mbEdit`/`evEdit` pattern already used by Awards/Memberships/Events (`app-setup.js`): an edit-id module var, an `if(editId){update}else{push}` branch in each save handler, and an `✎` button per card wired through the existing global delegated click listener.

**7. `board.js` reward literals → `VALUES` table.** Board-prep task completion now reads `VALUES.board.xp`/`.g` (a new flat single-tier entry in `constants.js`, since board tasks — unlike quests/dailies — have no difficulty concept to build a picker around) instead of hardcoded `grant(20,10,...)`. Also corrected a stale figure in the old `NEXT-SESSION-PROMPT.md` note itself (it cited `grant(8,4,...)`, which actually belongs to a different call site — the boss-checkpoint grant in `events.js` — not this one) and added a `+20 XP · 10 pts` preview tag to undone board cards, matching the `diffTag()` pattern quests/dailies already have.

**8. `profile.js`'s `spark()` deleted**, both Vitals sparkline call sites (resting pulse, hemoglobin) switched to the shared `miniSparkline(vals,90,22)` (`state.js`) — with a real behavioral fix along the way: `miniSparkline` doesn't null-filter its input the way `spark()` did internally, and both call sites pass raw `v.map(x=>x.pulse)` arrays that can contain `null` entries (any vitals row can have some fields unset), which would have silently corrupted the sparkline's min/max range. Fixed by filtering nulls at the call site before passing in.

**9. FM — detraining/long-layoff awareness in `computeTarget()` (rated HIGH value by the v209 audit).** Tier 1 ("adaptive") now checks days since the last logged set on an exercise; 7+ days (matching `plan.html`'s own "missed a week or more" copy) short-circuits the normal trend/stall math (comparing across a real gap isn't a meaningful trend) and returns a flat-percentage-reduced target instead — ~10% off weight (rounded to the nearest 5 lb), 2 fewer reps, or a scaled time/hold target, explicitly not a fitted detraining curve, just refusing to hand back the same progressive-overload number as if no time had passed. Verified live: a squat logged 14 days ago at 8 reps × 135 lb now returns "6 reps × 120 lb (easing back in)" instead of the old "10 reps × 135 lb" it would've produced by only reading the stale trend.

**10. FM — proactive PT-day scheduling, the largest single change this session.** New `S.ptDays` state (`{default,weekOf,week}`, same 3-layer cascade as `S.gymAccess`) plus `ptDayForDate()`/`weekPtPatternForEditing()`/`confirmWeekPtPattern()`/`saveDefaultPtPattern()` (`training.js`), mirroring the existing gym-access helpers exactly. `assignWeekSessions()` now claims declared PT days *before* either session pool runs, so a PT day never also gets a duplicate hard FM session (`plan.html`'s own copy already promised this: "count it as that day's session and don't double up"). `spaceOutHardDays()` extended to treat a PT day as "hard" for adjacency-spacing purposes too — but as a *fixed* side, never a swap target, since it's the user's real external schedule, not something the algorithm can move. New "ROTC PT Days" toggle card on the Plan tab (`renderPtDayUI()`), same day-toggle/confirm/save-default UI as Gym Access.
   - This required updates in 5 more places that read `assignWeekSessions()`'s output and would otherwise crash or silently misbehave on the new `"pt"` session value: `planForDay()` (new `PT_DAY_META`), `todaysPlan()` (a new `ptOnDay()` helper so "was today logged" checks `S.ptLog` instead of the always-empty-for-PT `S.workouts`), `renderCoachToday()` and `dawnSessionHtml()` (both needed a dedicated PT-day branch before their `SESSIONS[skey]` lookups, which would otherwise throw on an undefined key), and `trackMissedSessions()`/`weekTrainingStats()` (both previously checked `workoutOnDay()` only, which would have permanently flagged every PT day as "missed" even when logged).
   - Verified live via a throwaway Playwright script: `assignWeekSessions()` with declared Tue/Thu PT days correctly excludes both from FM assignment with no double-booking; `planForDay()`/`todaysPlan()`/`render()` all complete with zero page errors on a PT day. One known, accepted limitation: a dense input (gym M/W/F + PT Tu/Th, tested explicitly) leaves 5 of 6 weekdays "hard" with only Saturday soft — full adjacency separation is mathematically impossible at that density (pigeonhole), so `spaceOutHardDays()`'s existing "verified zero adjacent hard days across every pattern" guarantee no longer holds in that narrow case. This reflects real physical reality (that schedule genuinely is almost all hard days) rather than an algorithm bug, and the two things actually asked for — no double-booking, best-effort spacing — both hold.

**11. FM — `detectOvertrainingTrend()` now reads `S.ptLog`, not just `S.workouts`.** A cadet training mostly via logged unit PT could never trip the deload advisory before. `S.ptLog` only has a single whole-session `intensity` (1-10) per entry, not the per-exercise `effort`/`reduced` signals a workout has — so the honest fix reuses the *one* comparable real signal (`intensity>=9`, the same self-rated-difficulty scale as a workout's RPE) rather than inventing a proxy for signals PT logging doesn't capture. Verified live: 4 recent PT-only entries, 3 rated 9/10, correctly returns a struggle signal (rate 0.75).

**12. Quiz — Timed Mode.** New toggle on the Quiz tab (off by default); when on, each question gets a real 15-second countdown (`#qmTimer`, same `setInterval`+`Date.now()` idiom as `test.js`'s Cipher Desk/Fire Mission sprints), a run-out auto-submits as a miss (feeding the same SRS-miss pipeline a manual wrong answer does) with a distinct "⏰ Time's up" message instead of "❌ Not quite." Addresses the single biggest board-readiness gap the v209 audit found: real boards are rapid-fire verbal, not untimed multiple-choice.

**13. Quiz content gaps closed.** New **CBRN/NBC** category (8 questions: MOPP levels, chemical agent categories, the M50 mask, immediate decon) — previously zero coverage. **TLP depth**: added the 6 previously-untested steps (3 through 8) to the `opords` category, which only tested steps 1–2 before. **Land nav depth**: added major/minor terrain-feature ID (hill, saddle, depression, draw, spur, cliff), pace count, and resection/intersection to `landnav`, none of which existed before. Bank is now 21 categories; the "pass all quizzes" boss objective self-heals its target count automatically (existing mechanism, no change needed).

Verified via two throwaway Playwright scripts (deleted after use, per this project's standing pattern): one exercising every new function directly in-browser (layoff pricing, non-layoff trend logic still intact, overtraining-via-ptLog, `assignWeekSessions`/`planForDay` with PT days, `donationLevel()`'s full curve, `grantPathXP()`'s live output, the shortened Memory retention ladder, confirming `renderSynthesisChain` is fully gone) with zero page errors; one running the app's real `load()`/`seedSkillsIfEmpty()` migration path against two simulated old saves (one with Memory retention at a still-valid level, one at a since-impossible level) confirming clamp-down-only, peak/history preservation, and Blood Donation's clean addition.

That same migration test's duplicate name+cat sweep found **10 pre-existing duplicate `name|cat` pairs already in `SEED_SKILLS`**, none of them touching anything added this session (confirmed "Blood Donation|roots" isn't among them): `Personal finance|personal`, `Penetration testing methodology|technical`, `Legal literacy|personal`, `Threat Intelligence Analysis|technical`, `Cyber Operator|technical`, `SIEM & Log Management|technical`, `Cognitive Resilience|cognitive`, `Tax Strategy|personal`, `Debt Management|personal`, `Privilege Escalation|technical`. Not investigated this session — flagged in `NEXT-SESSION-PROMPT.md` as a loose end for whenever `skills-data.js` is next touched.

Full ship checklist: `npm run build` → OK (118,105 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525`. `SKILL_LADDER_VER` bumped 118→119 (Memory retention ladder shrink). SW bumped to `operations-v210`. `npm run package` → produced `dist/operations.zip`.

### v211 — the 10 pre-existing duplicate name+cat skill pairs from v210's migration test, fixed

**Files changed:** `src/core/skills-data.js`, `src/core/migration.js`, `sw.js`.

Wyatt asked to look into the 10 duplicate `name|cat` pairs v210's migration test surfaced (flagged, not investigated, in that session). Dispatched an Explore agent to read every pair's full context (line numbers, `setKey`/`rarity`/`parent`/`fadeDays`, content) and determine bug-vs-intentional for each. Before trusting the report, verified the actual severity with a live smoke test: `skSeedOf(name,cat)`'s Map is first-wins, so the theory was "one entry is dead/unreachable" — but the test showed something worse: `mergeNewSeedSkills()`'s duplicate check (`have=new Set(S.lifeSkills.map(s=>s.name))`) is a **snapshot taken once before the loop and never updated inside it**, so both same-named `SEED_SKILLS` entries independently pass the `!have.has(s.name)` check and both get pushed as separate live skill objects. Confirmed live: all 10 pairs had `liveCount:2` on a fresh seed — every affected save has a redundant, confusing duplicate card on the Skills tab for each of these 10 names, both showing identical (and for one of them wrong) guidance/ladder/rarity content.

The 10 pairs split into two patterns: 8 are same-tier accidental collisions between two independently-authored, unrelated chains (e.g. two different "Legal literacy" Uncommons, two different "Privilege Escalation" Uncommons — one from the original tree, one from a later `tech2`/`pers2`-prefixed expansion tree that didn't check for name clashes). 2 are cross-tier "flagship name reused at a higher tier" collisions — a Legendary "Cyber Operator" and a *separate* Mythic "Cyber Operator" (the `tech2` tree's capstone), and a Rare "Cognitive Resilience" and a separate Legendary "Cognitive Resilience" (the `cog2` tree's capstone) — both clearly a deliberate naming choice (reusing a strong title for a tier-up capstone) that broke on the app's name+cat keying.

Asked Wyatt directly (via `AskUserQuestion`) rather than guessing on two real judgment calls: how to disambiguate the 2 capstone names (chose: keep the flagship name on the lower tier, prefix the tier name onto the higher-tier capstone — "Mythic Cyber Operator" / "Legendary Cognitive Resilience"), and whether it's safe to delete a shadowed duplicate's live card outright (chose: yes, but only if genuinely untouched — `currentLevel===0 && peakLevel===0` — otherwise keep both rather than guess which is "real").

**Fix, two halves:**
1. **Seed-side:** renamed the 10 shadowed `SEED_SKILLS` entries to distinct names (checked each against the full skill list first — zero collisions): "Personal finance"→**"Military Pay & Allowances"**, "Penetration testing methodology"→**"Pentest Methodology Fundamentals"**, "Legal literacy"→**"Legal Systems Navigation"**, "Threat Intelligence Analysis"→**"Advanced Threat Intelligence Analysis"**, "Cyber Operator" (Mythic)→**"Mythic Cyber Operator"**, "SIEM & Log Management" (Rare)→**"Advanced SIEM & Log Analysis"**, "Cognitive Resilience" (Legendary)→**"Legendary Cognitive Resilience"**, "Tax Strategy"→**"Wealth-Building Tax Strategy"**, "Debt Management"→**"Wealth-Building Debt Management"**, "Privilege Escalation"→**"Network Privilege Escalation"**. Re-ran the full duplicate sweep after: `seedDupeCount:0`.
2. **Live-save side:** new generic dedup pass in `mergeNewSeedSkills()` (`migration.js`), placed right before the existing "add new seed skills" loop. Groups live skills by `name|cat`; for any group of 2+, if at most one member has ever been touched (`currentLevel>0||peakLevel>0`), collapses the group down to just that one (or the first, if neither was touched) — this isn't hardcoded to the 10 names above, it's a general safety net for this whole class of bug. If 2+ members show real progress, does nothing (can't tell which is "real" without guessing, so no data is ever silently dropped).

Verified live via two throwaway Playwright scripts (deleted after use): one confirming a fresh seed has zero seed-level duplicates and exactly one live skill per old/new name pair; one simulating a real pre-rename old save with actual duplicate objects across three scenarios — both duplicates untouched (correctly collapses 2→1, new tier-up skill seeds fresh at 0), one duplicate with real progress (`currentLevel:3,peakLevel:3` correctly preserved exactly, verified only after fixing a test-harness gap — forgetting `pyramidResetApplied:1` on the simulated old save, the same gap hit during v210's migration testing, made a correct result look like a bug at first), and both duplicates with real progress at different levels (ambiguous case: confirmed nothing is dropped, all entries survive untouched).

Full ship checklist: `npm run build` → OK (118,133 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525` (renames only, no skills added/removed). `SKILL_LADDER_VER` left at 119 — no existing *reachable* skill's ladder/tier/guidance content changed, only names of previously-shadowed skills. SW bumped to `operations-v211`. `npm run package` → produced `dist/operations.zip`.

### v212 — full-project audit (6 parallel agents), ~29 confirmed bugs fixed across correctness, security, and performance

**Files changed:** `src/core/state.js`, `src/core/migration.js`, `src/core/skills-core.js`, `src/core/skills-data.js`, `src/core/app-setup.js`, `src/core/events.js`, `src/core/insights.js`, `src/core/auto-level.js`, `src/tabs/skills.js`, `src/tabs/today.js`, `src/tabs/awards.js`, `src/tabs/weight.js`, `src/tabs/records.js`, `src/tabs/aft.js`, `src/tabs/plan.js`, `src/core/training.js`, `src/tabs/cardgame.js`, `src/tabs/test.js`, `sw.js`.

Wyatt asked for "a full audit of the app looking at everything, code, gaps, anything else — and then fix them." Dispatched 6 parallel Explore agents, each with a distinct subsystem: FM/training, quiz/test, skills/pyramid core mechanics, tree/garden/trophies visualization, awards/profile/weight/records/board, and a cross-cutting technical pass (security, performance, PWA, migration, state integrity). Together they returned ~60 findings. Triaged: every Critical and High-severity finding (~29 confirmed, real bugs — not design questions) fixed this session; Medium/Low findings queued in `NEXT-SESSION-PROMPT.md` rather than rushed, consistent with this project's established audit-session pattern.

**The two most severe findings, both in `state.js`:**
1. **`load()`'s catch-all could silently factory-reset a save, and that reset propagated to the cloud file and TOC.** Several array-typed fields (`lifeSkills`, `dailies`, etc.) were merged from the raw save with no shape guard — a single corrupted field (a truncated cloud/TOC sync, a hand-edited backup) threw deep inside a later `.forEach`/`.push`, and the catch swallowed it and returned a pristine `DEFAULT`. `save()`'s `cloudWriteDebounced()`/`tocWriteDebounced()` then flushed that empty state over every synced copy within moments, with a "Backup restored" toast telling the user it worked. Fixed with a one-time coercion pass right after the initial merge: any field `DEFAULT` declares as an array is forced to `[]` if the raw save didn't have a real array there, so a malformed field degrades to empty instead of nuking the whole save.
2. **The daily streak read its own prior-day state one cycle before it was written.** `checkDailyReset()` stored "was yesterday complete" in `S._yesterdayComplete`, but by the time the NEXT day's reset read it, it actually described the day *before* that — so real perfect days were never credited on time, and days the user did nothing on *were* credited, silently, since streaks were introduced. Root cause: the value was always available fresh at the top of the very call that needed it (dailies aren't reset to `false` until the end of that same function), so it never needed to be persisted across calls at all — removed the stored field entirely, replaced with a local variable computed once per call.

**Two more migration bugs, in `mergeNewSeedSkills()` (`migration.js`), that permanently blocked most of the Mythic-tier pyramid content:** the function's duplicate-check (`have`) and its existing-skill lookups were keyed by `name` alone, but 21 `SEED_SKILLS` entries share a name across two different categories (e.g. "Movement Quality" exists in both `physical` and `physiological` with different ladder lengths — confirmed with a live save: `physical:14, physiological:8`). A name-only key meant a live skill in one category could get clamped to the OTHER category's max level, or a seed in the second category could be treated as "already have it" and never get created — permanently capping progress and blocking the synthesis sets those seeds belonged to. Fixed by keying every lookup on `name+"|"+cat`. Verified live: all three sampled duplicate-name pairs now correctly show both categories present with their own distinct ladder lengths.

**A ~400x performance fix.** `skReadyToCombine()` (called every `render()`, i.e. dozens of times per session) did an O(n) `Array.find()` per member of every synthesis set across all ~2,500 sets and ~12,500 skills — measured by the audit at ~7.4 seconds of blocking main-thread work per call at full skill scale, with `catPyramidCompletion()` (called once per Path, also every render) and `skills.js`'s Pyramid-tree mastery headers adding roughly another 1.9-3 seconds each via the same pattern. Fixed with two memoizations: `skSetMembers()` now caches a `setKey → members` Map built once from `SEED_SKILLS` (immutable at runtime, same premise as the existing `skSeedIndex()`), and a new `skLiveIndex()` builds a `"name|cat" → liveSkill` Map once per render (invalidated at the top of `render()` in `state.js`), replacing every O(n) `.find()` in `skSetMasteredCount()`, `skCombineSet()`, `skReadyToCombine()`, `catPyramidCompletion()`, `skills.js`'s `pyrLiveOf`, and `today.js`'s synthesis-ready check. Measured after the fix: `skReadyToCombine()` 18ms, all 10 Paths' `catPyramidCompletion()` combined 10ms — both down from multi-second.

**A stored-XSS in the Weight ledger** (`weight.js`) — the `status` field (both the keystone card and every ledger row) was interpolated without `esc()`, and `importWeightLedger()` didn't constrain it to the 4 legal values, so a malicious/corrupted imported or cloud-synced ledger file could inject a payload that fires on every render, no need to even visit the Weight tab. Fixed: escaped both interpolation sites, and `importWeightLedger()` now whitelists `status` to `open`/`standing`/`kept`/`broken`.

**Cloud/TOC/import hardening, several related gaps:** all four save-adoption sites (`linkCloudFile()`'s existing-file path, `cloudInit()`, `tocInit()`, the file-import handler in `events.js`) accepted any `typeof x==="object"` value, which is also true of arrays — an array-shaped file silently produced a garbage state via `Object.assign`. All four now reject arrays explicitly. `linkCloudFile()` additionally used to swallow a JSON-parse failure on a non-empty file and then immediately overwrite that file with current local state a few lines later — the "unreadable file" case is now distinguished from "empty file" and aborts the link with a warning instead of silently destroying whatever was actually in the file. Separately, `cloudFlush()`/`tocFlush()` cleared their "dirty" flag unconditionally after a write completed — since a cloud-file write routinely takes over a second, a second change arriving mid-write could have its own already-scheduled flush silently skipped once the first flush's completion cleared the flag. Fixed with a generation counter on each: a flush only clears "dirty" if nothing newer has queued since it started. The footer's "synced to TOC" label was also gated on `_tocPresent` alone (TOC merely answering a health check), not `_tocDataConfirmed` (writes actually enabled) — it could claim sync was active for a whole session where every write was silently being refused; now gated on both.

**Paused daily orders were inconsistently excluded.** `events.js`'s own Perfect-Day check already excluded `paused` orders, but `checkDailyReset()`, `readiness()`, and `render()`'s order-count didn't — so pausing even one order (a supported, first-class action) meant a genuine 100%-of-active-orders day still triggered a "Perfect Day" toast while the next day's streak check saw the still-undone paused order and refused to increment, permanently. All three now exclude paused orders consistently.

**`insights.js` compared a 0-100 percentage against a 0.7 threshold** (`insightStreakVsAft`'s `avgPct`, stored as an integer everywhere else it's consumed) — meaning any 2-week window averaging as little as 1% daily-order completion was bucketed as "consistent," either making the check permanently inert or, worse, letting it publish "a real pattern in your logged data" backed by a "consistent" bucket that actually averaged single-digit completion. Fixed to compare against `70`.

**FM fixes:** the card-game workout mode (`cardgame.js`) misread the AFT-anchor tier's target string as a raw rep count via a generic leading-number regex — for a deadlift with no logged sets yet, it read the AFT max WEIGHT (e.g. 340 lb) as the rep count, producing a threshold of up to 1,020 reps for one exercise group; for push-ups, it read a 2-minute test TOTAL as a per-set number. Fixed to treat that tier as "no usable data" and fall through to the existing safe default. Entering `"0"` for AFT deadlift/push-ups (`aft.js`, both the manual-entry form and the guided-mock capture) was silently treated as "not attempted" (`parseInt(x)||null` — `0` is falsy) rather than a real, possibly-failing score, hiding the exact safety signal (an event below the 60-point floor) the form exists to surface; fixed with a proper empty-string check. Dawn's compact session card (`plan.js`) showed all 4 run variants side-by-side on a "pick-one" day instead of resolving today's actual pick via `pickRunIndex()` like Coach Today already does, contradicting the "you do one run today, not all of them" copy shown right next to it. The new PT-day missed-session tracking (`training.js`) could retroactively reconstruct a PRIOR week's schedule from the DEFAULT pattern once a newer week's pattern was confirmed (the gym-access/PT-day cascade is only reliable for the currently-confirmed week by design) — producing a false "you missed this session" accusation for a day the user actually handled correctly via a declared PT day; capped the lookback to the current week only.

**Quiz/skill-ladder fixes:** the Go/No-Go game's 25-trial format means every achievable accuracy is a multiple of 4%, but its level thresholds (99, 98, 97, 94, 92, 90, 85 — none multiples of 4) meant levels 3, 5, 7, 8, and 9 of "Attention / sustained focus" could never be reached by any real score, provably, no matter how well the player did — rebuilt the thresholds (and the skill's ladder/guidance text) using only the 11 accuracies 25 trials can actually produce. The "ROTC knowledge (quizzes)" skill's leveling formula (`auto-level.js`) was still hardcoded against "16 quiz banks" — the exact staleness bug already fixed once for the sibling boss objective in `quizzes.js`, now found unfixed in the piece that actually drives skill leveling; a cadet who'd passed 16 of the live 21 categories was shown as fully "Knowledgeable" while 5 real categories (including CBRN and Radio Comms) sat untouched. Fixed to scale by *fraction* of the live bank size, mirroring the boss objective's existing self-heal pattern; rewrote the skill's ladder text in fractions (~1/8, ~1/3, half, …) so it can't go stale again as the bank keeps growing. The N-back game's difficulty picker only ever offered 1-4-back even though `scoreToLevel` has always had a real `n>=5` branch worth L8 — added the 5-back option so the documented honest ceiling is actually reachable.

**Skill-engine correctness fixes:** `skEffectiveLevel()`/`skFadeState()` didn't clamp a negative elapsed time (a `lastQuestTs` in the future — a device-clock error, or a cloud/TOC restore of a save written on a device with a fast clock) — `Math.floor` of a negative ratio rounds toward `-Infinity`, so the computed "intervals since last touched" went negative and got ADDED to the current level instead of subtracted, producing an effective level ABOVE the real ladder length and permanently blocking every rung tap (`skReachLevel`'s no-op guard is `level<=eff`) until the bad timestamp's future date actually arrived. Now clamped at 0. `skDaysLeft()` had the same unbounded-negative problem for a very stale skill (returns e.g. `-1064` for a 3-year-abandoned L1 skill) — since several UI sites inconsistently guard `days<=0` and some just print the raw number ("Fades in -1064 days"), and ascending sorts by this value (the Decaying-soon column, Weekly Queue) ranked the MOST abandoned skills as the safest, monopolizing the "at risk" slots with exactly the skills that least need surfacing there. Now floored at 0. `skReadyToCombine()` used to treat a missing live skill (deleted mid-session after being started/combined) the same as "never started," resurfacing a dead Combine button with no feedback when tapped; `skCombineSet()`'s two other silent-failure paths now show a toast instead of doing nothing. The Pyramid-tree browser's lazy-render `toggle` listener (`skills.js`) was re-added to the same static `#skList` container on every single `render()` call with no matching removal — a real, unboundedly-growing listener leak for the whole session, the same class of bug `tree.js`'s pan/zoom cleanup already guards against, just missed here; now tracked and removed before each re-add.

**Pyramid-reset migration cleanup.** The one-time "wipe progress on skills newly assigned to the pyramid" migration tracked completion with a per-skill boolean (`pyramidResetApplied`) on ~12,500 skills — permanently bloating every save by ~300KB for a migration that, once complete, never needs to run again — and its guard checked `!live.pyramidResetApplied`, never actually comparing against `PYRAMID_RESET_VER`, so bumping that constant to re-trigger a sweep for future pyramid-tagged content would silently do nothing. Collapsed to a single save-level `S._pyramidResetVer` flag: the sweep still runs against the per-skill flags once (for any save that predates this, preserving whatever was already reset and whatever wasn't), then deletes them and records completion once at the save level. Verified live against a simulated real old save (12,496 skills carrying the per-skill flag, one with real progress): saved 336,544 bytes, real progress preserved exactly, zero page errors.

**Awards/export fixes:** ROTC Record's position semester fields (`startSem`/`endSem`) were interpolated without `esc()` — directly reachable by just typing into the normal "Add position" form, no import needed; also sorted by raw string comparison (`"SP2024" > "FA2024"` alphabetically, even though Fall postdates Spring) — the same bug class already fixed once for GPA-history term sorting, found unfixed in a sibling feature built afterward; both fixed (escaped, and switched to the existing `termSortKey()` helper). Year fields across Awards/Events/Academic Honors/ROTC competitions & camp results were also unescaped — fixed. The Volunteer CSV export (`records.js`) referenced fields (`date`, `name`/`activity`) that don't exist anywhere in the real volunteer-entry schema (`year`, `org`, `hours`, `note`) — the export was silently near-empty; fixed to the real fields. All CSV exports (`downloadCSV`) now defuse a cell starting with `=`/`+`/`-`/`@` (a live-formula risk in Excel/Sheets) with a leading quote, since these exports are explicitly meant to be shared with a battle buddy or cadre.

**Miscellaneous:** a CSS selector built by interpolating a boss id directly into a query string (`events.js`) would throw a `SyntaxError` on an id containing a quote/bracket from a malformed save — rewritten to scope via `.closest()`/`.querySelector()` instead of building a selector string. The study-plan feature's test-date math (`today.js`, `test.js`) parsed a bare `"YYYY-MM-DD"` `<input type=date>` value as UTC midnight rather than local midnight — silently shifting the effective date back a day for any timezone west of UTC (nearly the whole target userbase); anchored to local noon first, the same fix already used for quest snoozing.

Verified via three throwaway Playwright scripts (deleted after use): a broad functional smoke test covering every fix above directly in-browser (cross-category migration keying, the streak fix, paused-order handling, the performance numbers, clock-skew clamps, all 10 Go/No-Go levels now reachable in sequence, quiz fraction-leveling, the Weight XSS escaping confirmed neutralized, AFT "0" preservation, the card-game fix) with zero page errors; a dedicated migration test confirming the pyramid-reset storage collapse against a simulated real old save. Full ship checklist: `npm run build` → OK (118,379 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525`. `SKILL_LADDER_VER` bumped 119→120 (Go/No-Go and ROTC-knowledge ladder text changed). SW bumped to `operations-v212` (also carries the SW-level fixes: only cache genuinely successful same-origin responses, never a mid-deploy 404/captive-portal page; a 2.5s network-vs-cache race on app-code fetches so a slow connection doesn't leave the app blank when a good cached copy exists; a missing/uncached asset now resolves to a real 404 instead of silently substituting all 21MB of `index.html`). `npm run package` → produced `dist/operations.zip`.

### v213 — the rest of the v212 audit queue cleared ("fix everything else") — ~35 Medium/Low findings

**Files changed:** `src/core/skills-core.js`, `src/core/skills-data.js`, `src/core/migration.js`, `src/core/state.js`, `src/core/constants.js`, `src/core/events.js`, `src/core/auto-level.js`, `src/core/app-setup.js`, `src/core/insights.js`, `src/core/tree.js`, `src/tabs/skills.js`, `src/tabs/awards.js`, `src/tabs/test.js`, `src/tabs/quizzes.js`, `src/tabs/plan.js`, `src/tabs/aft.js`, `src/core/training.js`, `src/tabs/log.js`, `src/tabs/records.js`, `src/styles/main.css`, `sw.js`.

Wyatt asked to fix everything left in the v212 audit's Medium/Low queue. Went through it systematically, file by file, fixing every item rather than triaging further — nearly all were genuine confirmed bugs, not design forks, so no `AskUserQuestion` stops were needed this pass.

**Skill/pyramid data quality:**
- **6 Legendary ladders had 3 blank rungs each** (`Foundation of Iron`, `Endurance Engine`, `Body Composition Mastery`, `Combat Conditioning`, `Movement Quality` [physical], `Close-Quarters Combat Mastery`) — `levels[11..13]` were empty strings, tappable rungs with zero ability description, inflating each ladder's claimed length to 14 when only 11 were ever really authored (confirmed against a sibling Legendary card, `Iron Kit`, which has real, full content at every one of its 11 levels — these 6 were genuine outliers, not an intentional Legendary-tier pattern). Honest fix: truncated to the real 11 levels rather than inventing 18 new ability descriptions. Verified via a migration test with a simulated old save at `currentLevel:12, peakLevel:13` — both correctly clamp to the new max of 11, no data loss, no crash.
- **9 technical skills rendered one rarity tier too high** (`Linux / command line`, `Networking`, `Programming` ×3, `Cybersecurity fundamentals`, `DevOps / containerization`, `CTF / competitive security`, `Machine learning / AI fundamentals`) — all are real Uncommon-tier synthesis targets (`tech_u_*` setKeys) that never got an explicit `rarity` field, so `skRarity()`'s level-count fallback misjudged their 8-9 rung ladders as Rare. Added the explicit `rarity:"uncommon"` field each was missing, matching how every other pyramid skill declares it.
- **23 `auto` skills were forced to "Joker" rarity and rendered twice** — `skRarity()` treated `sk.auto` as sufficient for Joker status on its own, but most auto skills (Reaction speed, Push-ups, Integrity, etc.) have a real pyramid `setKey` and tier; masking that ALSO meant `skills.js`'s Wildcards-deck filter (any `auto||joker` skill) double-rendered each one — correctly under its real Path deck, and again mislabeled as a Joker under Wildcards. Fixed `skRarity()` to check explicit/seed rarity first and only fall through to Joker when there's genuinely no pyramid data at all (Blood Donation is the one real skill this still applies to); matched the Wildcards filter to the same criterion. Verified live: Wildcards deck membership dropped from 24 to 1 (just Blood Donation), and Reaction speed now correctly shows "Rare" and no longer appears in both decks.
- **70 skill entries had `maintain`/`advance` shorter than `levels`** — guidance text silently vanished exactly at the peak level, the one level maintenance instructions matter most. Rather than hand-authoring ~140 bespoke lines across 70 different skills, fixed the actual gap: when a level's `maintain` text is missing, the "Hold" block now falls back to the skill's own `roadmap[]` entry for that level (roadmap always matches `levels.length`, unlike maintain/advance) — real content the skill already has, not invented text.
- **An orphaned seed skill (renamed/removed from `SEED_SKILLS` with no matching `RENAMES` entry) would crash the ENTIRE Skills tab render**, not just its own card — `leafCard()` dereferenced `sk.levels.length` with no guard, and since the whole tab is built as one big HTML string, one broken card took every other skill's card down with it. Added guards in `leafCard()`, `skQuest()`, and `skReachLevel()` so an orphaned skill degrades to "skip this card" / a clear toast instead of a blank tab. Verified live with a simulated orphaned skill present during a full render — zero errors, every other card still rendered.
- `skLeafColor()` divided by zero for a hypothetical 1-level skill (no current seed has one, but a user-created custom skill could) — now treats a 1-level skill as fully lit once started instead of computing `rgb(NaN,NaN,NaN)`.
- `syncSkillsFromActivity()`'s `(S.aft||[])[S.aft.length-1]` guarded the indexing but not the `.length` dereference itself, which still throws if `S.aft` is ever undefined — fixed to snapshot the fallback array first.
- A dead `grace>0` branch and one place reading `sk.fadeDays` without the `||30` fallback everyone else uses — both cleaned up.
- `sk.history.push()` was unguarded at 9 call sites across `skills-core.js`/`auto-level.js` — rather than patching each one, backfilled `history:[]` centrally in `load()` (same pattern as the existing `peakLevel` backfill), so any save old enough to predate the field is covered everywhere at once.
- `getTierLabelForLevel(sk,0)` returns the FIRST tier's label (matches on `level<=t.upTo`, and 0 satisfies the first tier the same as level 1) — `trophies.js` already guarded this correctly, but the copy-to-clipboard handler in `events.js` didn't, so copying an unstarted skill's card claimed a tier it hadn't earned. Now shows "Unproven" for `eff<=0`.

**Visualization:**
- The `physiological` world (deliberately pushed further out as the crown's "apex") could render partially or fully off-canvas on wide/desktop viewports on first load, not fixed by "Reset view" either since it re-ran the same flawed fixed-constant math. Replaced the hardcoded `crownTopY-70` with a real computation from the actual slot positions and the halo's own worst-case radius, so the auto-fit adapts to wherever any world actually is instead of assuming they're all at the same distance.
- `_hearth()`'s emblem generator had the exact same unreachable-branch bug already found and fixed once in `_physical()` (`s>>2&2` can only ever be 0 or 2, never 1) — not propagated to check this sibling function at the time. Fixed to `&3`, restoring the 3rd intended visual variant.
- The "Erase all progress and start over" reset handler never called `seedSkillsIfEmpty()` (every other full-state-replace path — import, cloud/TOC restore — does), so Tree/Garden/Trophies/Skills showed "no skills yet" until the next full page reload instead of immediately re-seeding like a fresh install. Fixed.

**FM:**
- `renderPlanRec()` still hardcoded a 380-point AFT target regardless of `S.aftStandard`, visibly contradicting the neighboring `aftPrepCard` for anyone on the general (300) standard. Now derives 300/350 from `aftCtx()` like every other AFT-standard-aware surface.
- The app disagreed with itself on the real per-event AFT floor under Combat standard — `aftPrepCard` said 70, `renderAftStandardBar`/`showAftResult` said 60, for the exact same score on the same tab. The AFT's per-event floor is a flat 60 regardless of standard (only the TOTAL requirement varies); aligned all three to 60.
- `pickAftMode()`'s "haven't tested in 45+ days" check mixed a locale-formatted date string (`toLocaleDateString()`) against an ISO string, silently producing `Invalid Date`/`NaN` (never `>=45`) for any non-US locale. New AFT entries now also carry a real `ts:Date.now()`; the check prefers that when present.
- The taper window excluded the actual test day itself (`daysToTest>=1`, not `>=0`), so a hard session could still be scheduled on test day — the one day the taper exists to protect. Fixed to `>=0`. Verified live by forcing a hard day to fall exactly on a declared test date: intensity now correctly downgrades to moderate with `taper:true`.
- The day-hash exercise-suggestion rotation (`suggestedPoolIndex`) fragmented per-exercise-name logged history across equipment-eligible equivalents (e.g. alternating hand-release/knee push-ups by date), silently locking a consistent user out of `computeTarget()`'s adaptive tier for that slot since no single name ever accumulated the 2-3 consecutive logs needed to read a trend. Now prefers whichever pool candidate the user has actually logged before, falling back to the date-hash rotation only for a never-logged slot (preserving discovery/variety there).
- `baselineKeyFor()`'s `tempo|interval` regex matched ANY cardio exercise with that word in its name — "Rower intervals," "Stationary bike intervals," "Swim intervals," and the indoor bodyweight-circuit swaps all got bucketed under the 2-mile-run baseline despite none of them being running, letting an unrelated exercise's monthly trend get attributed to the wrong one. Now excludes rower/bike/swim/indoor explicitly; treadmill variants correctly stay included since a treadmill genuinely is running.
- `S.exChoice`/`S.gymAccessLive` grew one key per day forever with no cutoff, unlike `S.missedTraining`'s existing 28-day trim in the same file — added the same trim to both.

**Quiz/Test:**
- `render()` (triggered from dozens of unrelated actions app-wide, several living on this same Test tab — an SRS deck edit, a study-plan checkoff) unconditionally rebuilt the whole Test tab list on every call, including every `#stage-X` div — silently detaching a live in-progress construct (a running Cipher Desk pad, a ticking Fire Mission countdown) or a just-shown result screen, with no visible sign anything happened. Rewrote `renderTests()` to diff per-card: a stage with real content (a reliable generic signal of "mid-play or showing a result worth keeping") keeps its live DOM subtree completely untouched — only the surrounding chrome (level/best/last) refreshes in place. The Start button is handled by a body-level delegated listener, not a per-node handler, so it stays functional either way. Verified live: injecting a marker into a stage div, forcing an unrelated `render()`, confirming the marker survives.
- The "pass all quizzes" boss objective's stale-count self-heal only fired reactively inside `finishQuiz()`'s first-pass branch, so a bank that grew AFTER the user had already passed everything (already showing "complete") stayed stale indefinitely — passing a brand-new quiz, the one action that would trigger the fix, is exactly what a "complete" badge discourages. Extracted into `syncQuizBossObjective()`, now also called from `renderQuizzes()` (every Quiz-tab visit), so it self-heals proactively too. Verified live: deliberately staled the objective, called `renderQuizzes()`, confirmed it self-corrected.
- Quiz question/answer shuffling used the statistically-biased `sort(()=>Math.random()-0.5)` instead of the correct Fisher-Yates helper (`shuffleInPlace`) already in the codebase (used elsewhere for Comms Relay/Sentry) — switched both to it.
- The Reading test had no minimum-elapsed-time floor (unlike Comms Relay's `Math.max(0.4,...)` guard right next to it), so an instant Begin→Done tap could fabricate an astronomical WPM that permanently overwrites the "Best" record and insta-maxes the Reading speed skill. Added a 3-second floor (longer than Comms Relay's, since a real passage realistically takes several seconds minimum even for a very fast reader).
- Signal Intercept's round 3 (`{pool:4, step:2}`) had `gcd(2,4)=2`, so the cycle only ever visited 2 of 4 symbols — a trivial alternation, easier than round 2, not harder as the difficulty curve intended. Changed to `step:3` (`gcd(3,4)=1`, visits all 4, and a step of 3 in a 4-symbol pool still visually reads as a "jump" pattern to a human even though it's mathematically a reversed step-1 cycle).

**Awards/import:**
- `records.js`'s `importSection()` had no shape validation beyond "not null/undefined" — a malformed file assigning e.g. a string into `S.awards` used to crash the very next `renderAwards()` call. Now requires an imported value to be an array whenever `DEFAULT` declares that key as one, skipping (and warning about) any key that doesn't match rather than corrupting state.
- All 7 of the app's add-or-edit forms (Awards, Memberships, Events, Academic Honors, ROTC positions/competitions/camp results) showed a false "✎ updated" success toast if the record being edited had been deleted (via its own ✕) while the form was still open — no data corruption, but a misleading message with no indication the edit was silently discarded. Fixed all 7 to show a distinct "that record was deleted — nothing to save" message instead.
- None of the 7 forms had any way back to "Add" mode except submitting (writing a possibly-unwanted edit) or reloading the page. Added a shared `ensureEditCancelLink()`/`hideEditCancelLink()` helper (`app-setup.js`) and wired it into all 7 — a small "cancel edit" link appears next to the Save button while editing, resets the form and edit-state without saving anything.

**Cross-cutting:**
- **Cloud/TOC/import had no recency arbitration at all** — TOC is documented to have "the final say" over a cloud file when both are present, but that was never actually safe: a TOC save left over from a stale session on another machine could silently overwrite fresher local or cloud data on boot with zero warning. `save()` now stamps `S._lastModified=Date.now()` on every write; `cloudInit()`'s and `tocInit()`'s automatic boot-time restores now only adopt a remote copy if it's at least as new as the current local state. (Left `linkCloudFile()`'s explicit "pick an existing file" flow un-gated — that's a deliberate user action, not a silent background one.)
- Several `document.getElementById(...).onclick=...` assignments in `events.js` were unguarded in a single-concatenated-script build, where a renamed/removed id anywhere would throw and blank the ENTIRE app (every module after it in the build order silently fails to load). Wrapped all of them in the same `if(_el)` guard the rest of the codebase already uses consistently.
- Fixed the `DEFAULT`↔`load()` schema drift the v212 audit flagged: `profile.gpaGoal` was missing from `state.js`'s hand-written second copy of the profile schema (now derives from `DEFAULT.profile` directly instead of a duplicate literal, so it can't drift again); `equipProfiles`' default seed still contained `"agility"`/`"battlerope"`, two tags removed from `EQUIP_TAGS` in v192 (now stripped from both the seed AND any already-existing profile, since `Object.assign` doesn't merge nested arrays); `healthImport`'s declared `DEFAULT` shape (`{lastImport:null}`) didn't match what's actually written (`{lastImport,latest,fields,history}`) — updated to match; `weightAppUrl`'s default was hardcoded a second time in `load()` — now reads `DEFAULT.weightAppUrl`.
- `insightWeightVsDeadlift()` could publish "a real pattern in your logged data" off as few as 4 loosely independent comparisons (4 coin-flip-style agreements hit that threshold by chance ~12.5% of the time) — added a separate, higher bar (`INSIGHT_MIN_TOTAL_STRONG=8`) for the strong claim specifically, while still showing the plain fraction at the existing lower floor. Also fixed a real pairing-dedup gap: each AFT test independently found its nearest weight entry with no de-duplication, so multiple AFT tests taken close together could all pair to the SAME weight reading, inflating how many genuinely independent observations the count represented.
- Daily habits link to a skill by NAME (`S.dailies[].linkedSkill`) — quests link by id and were unaffected — and the skill-rename migration (`RENAMES` in `migration.js`) never rewrote it, so a renamed skill's linked habit silently stopped feeding its fade timer with no error or visible sign. Added the rewrite to the same `RENAMES.forEach` loop.

**Deliberately not fixed this pass** (flagged, genuinely lower priority than everything above): a handful of additional `S.lifeSkills` filter passes in `skills.js` still run unconditionally every render over the `.levels` getter — smaller in magnitude than the `skReadyToCombine`-class fixes from v212, real but not urgent on its own.

Verified via several throwaway Playwright scripts (deleted after use, per this project's standing pattern) covering essentially every fix above directly in-browser, plus two dedicated migration tests (the Legendary-ladder truncation, and a forced-hard-day-on-test-date taper check) — all passed, zero page errors throughout. Full ship checklist: `npm run build` → OK (118,711 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525`. `SKILL_LADDER_VER` bumped 120→121 (the 6 Legendary ladders shrank from 14 to 11 real levels). SW bumped to `operations-v213`. `npm run package` → produced `dist/operations.zip`.

### v214 — improvement audit (5 parallel agents), ~35 findings fixed + a 785-skill ladder-content pass

**Files changed:** `src/core/init.js`, `src/core/constants.js`, `src/core/events.js`, `src/core/app-setup.js`, `src/core/migration.js`, `src/core/skills-data.js`, `src/styles/main.css`, `src/tabs/today.js`, `src/tabs/skills.js`, `src/tabs/skills.html`, `src/tabs/board.js`, `src/tabs/board.html`, `src/tabs/quizzes.js`, `src/tabs/aft.js`, `src/tabs/records.js`, `src/tabs/records.html`, `src/tabs/profile.js`, `src/tabs/profile.html`, `src/tabs/dailies.js`, `src/tabs/weight.js`, `scripts/regress.js`, `scripts/unit-checks.js` (new), `scripts/extract.py`, `.gitignore`, `CLAUDE.md`, `package.json`, `sw.js`; deleted `scripts/build.js`.

After two exhaustive correctness passes (v212/v213), Wyatt's next request ("audit everything to see what can be improved and how") called for a different angle than another bug hunt. Dispatched 5 parallel agents across UX/design, content quality, feature-workflow completeness, onboarding/first-run, and code architecture/maintainability. All 5 returned real findings; two things were explicitly confirmed as *already working well* and needed no action — the AFT/FM adapt loop, and the v203 SmartFocus consolidation (no new competing "what needs attention" algorithm had crept back in). Findings were synthesized and presented to Wyatt via `AskUserQuestion` rather than built blind, since "what's mediocre and what's the opportunity" carries far more subjective judgment than a bug list. He chose to fix everything except two items explicitly flagged as large, dedicated-session-worthy features (see "Queued, not built" below) — those two were logged only, not scoped or built, per his second answer.

**UX/design (from the UX audit):**
- Nearly every text input was styled under the 16px threshold that triggers iOS Safari's zoom-on-focus — a fix already existed in exactly one spot (`.mockaft-input-row`) but was never generalized. Added a single `@media (max-width:560px){ input,select,textarea{font-size:16px!important} }` floor at the end of `main.css` rather than hand-editing ~20 scattered rules — deliberately uses `!important`, scoped narrowly to mobile widths and to `font-size` only, so it beats every current *and future* sub-16px rule regardless of its own specificity.
- 4 destructive deletes (Oaths, Trials, Rewards, Board tasks) had no `confirm()` at all, unlike every other delete path in the app (a comment in `events.js` even documented a related delete being "superseded" without the guard carrying forward). All 4 now confirm.
- Tap targets on 6 icon-button classes (`.del`, `.hb-del`, `.mb-edit`/`.mb-del`, `.aw-edit`/`.aw-del`, `.ev-edit`/`.ev-del`, `.daily-move-btn`) were well under mobile guidelines with edit/delete pairs sitting close together — grew padding and widened the gap between paired buttons on each.
- Skills tab's 6-button toolbar was icon-only with meaning conveyed only via `title=` tooltips, which never appear on a touchscreen. Added short visible labels (Weekly/Assess/Gaps/History/Copy/Sync) and made the row wrap on narrow viewports.
- Today's ~27-card flow had no visual hierarchy — the Focus card (explicitly "the one thing to do right now") sat 13th, equal-weight with ambient cards like path summary and academic snapshot. Reordered so welcome-back/onboarding/streak-recovery/Focus/alerts lead, ambient/informational cards trail.

**Onboarding (from the onboarding audit — note the audit's own headline finding: this was NOT a blank-slate cold-open as its framing assumed; `DEFAULT` already seeds real starter quests/orders/bosses and a "Getting Started" card already existed):**
- The Getting Started card's visibility was gated on an all-or-nothing AND of 4 conditions — doing step 2 before step 1 permanently hid steps 3-4's guidance the moment step 2's own condition tripped true. Rewritten as 4 independently-tracked steps (`S.name!=="Cadet"`, `S.aft.length>0`, a completed daily order, a logged workout), showing only whichever remain.
- No signal distinguished "first ever open" from "reopened after months away" — the single highest-impact finding for this app's actual (irregular, ROTC-semester-driven) usage pattern. Added `S.lastOpenedTs`, snapshotted once per load in `init.js` before being overwritten, surfaced as a "👋 Welcome back — it's been N days" banner on Today when the gap is ≥14 days.
- The Skills tab's first screen showed the identical fully-expanded 12,525-skill browser to a brand-new user and a 2-year veteran, with no "start here" signal. Added a curated pointer (`#skStartHere`) surfacing up to 5 genuinely easy Common-tier skills spread across Paths, shown only while `started.length===0`.
- The card-suit/rank layer (every skill card shows a suit glyph and an Ace-King rank) was undocumented anywhere despite driving real UI — added one line to the existing pyramid `<details>` explainer confirming rank reflects each skill's place in its Path's target ordering (verified against `skills.js`'s actual rank-assignment sort, not guessed).
- Install and notification-permission nudges could stack alongside the Getting Started card on a brand-new session's very first paint — three separate "please configure something" asks at once. Both now deferred until `startHtml` (Getting Started) is no longer showing.
- `vo2Benchmark()` (`aft.js`) silently assumed age 20 when no birthdate was set, unlike the adjacent AFT event display which shows an explicit "set birthdate in Profile" warning — added the same visible caveat to this call site.

**Feature/workflow wiring (from the feature-completeness audit):**
- Quick PT Log (Today's dashboard shortcut) satisfied `workoutOnDay()`'s "logged today" check but had no `rpe`/`exercises[].effort`/`.reduced` fields, so `detectOvertrainingTrend()`'s `wStruggled()` silently counted it as a non-struggling session regardless of how hard it actually was — a real gap in the app's core adaptive promise for a plausible usage pattern. Added an optional 1-10 effort field to the Quick Log form, stored as `rpe` (the exact field name `wStruggled()` already checks), so it now participates in overtraining detection without turning Quick Log into the full structured-session form.
- Board tab, Quiz's per-category readiness view, and Profile's commissioning scorecard were 3 disconnected screens sharing a database with no cross-links. Parameterized `renderBoardReadiness()` and `renderCommReadiness()` to accept an optional target-element id (defaulting to their original ids, fully backward compatible) and called both a second time from `renderBoard()` into two new elements on the Board tab — wiring existing functions, not duplicating their logic.
- Board tasks had no due-date field and never appeared in Today's Upcoming timeline (which already aggregates 5 other date sources). Added an optional `due` field to the add-task form, shown on the task row, and a 6th `.forEach` in `renderUpcomingTimeline()` following the existing pattern.
- Quiz-pass "review" daily orders were permanent and unbounded — passing all 21+ bank categories over a long tenure accumulates that many forever-daily-orders alongside the real 7 defaults, a "day 400" problem invisible on a fresh save. Capped active review orders at 5; when at the cap, frees a slot from whichever active review order has zero tracked SRS misses left (genuinely mastered) rather than ever dropping one with real tracked gaps.
- Fading skills, SRS-due cards, and an approaching AFT test all had real "silent unless something needs attention" logic (`computeSmartFocus()`) but none of it could reach the user outside the app — only streak risk could, via the one existing notification channel. Added a second, 9am `scheduleDecayNotif()` reusing the exact same permission/scheduling pattern as the existing 7pm streak notification.
- Battle Buddy Report (the printable "everything" export) and Wall→Résumé (plaintext clipboard) had disjoint, incomplete field coverage — a cadet needed both to approximate a complete commissioning/OER-support document, and neither had quiz/board-readiness at all. Expanded Battle Buddy Report to include academic honors, ROTC record, memberships, qualifications, languages/clearance, current GPA, and a board-readiness line, reading the same underlying `S` fields Wall's `copyWallResume()` reads rather than duplicating its logic.
- Profile (one continuous ~140-line form) and Records (7 unrelated sections in one flat scroll) both restructured — Profile's GPA log/Measurements/Best Lifts/Language/Clearance/Milestones sub-groups now sit in collapsed-by-default `<details class="pf-section">` blocks (mirroring the pattern Plan's "⚙️ Setup" already proved); Records got a 6-tab subnav (Notes/Saga/Counseling/AAR/Checklists/Export) using new `.rsub`/`.rec-sec`/`data-rsub` classes — deliberately NOT reusing Wall's `.wsub`/`.wall-sec`, since Wall's switcher does a document-wide `querySelectorAll`, and sharing classes would make a Records subnav click also blank out whichever Wall section was open.

**Code health (from the code-architecture audit):**
- `scripts/build.js` (a legacy Node build script, unused by any npm script) had silently drifted 2 tabs behind `scripts/build.py` (missing Garden and Trophies entirely) — exactly the failure mode CLAUDE.md's own comments warn about, and it had already happened. Deleted it; fixed the one stale reference in `scripts/extract.py`.
- CLAUDE.md's own orientation facts were badly stale: "Current version: v91" (actually v213), `skills-data.js` "~1,800 lines" (actually ~102,800), and `cardgame.js` absent from both the file-layout tree and the "where things live" table despite being a real, wired-in module. All corrected; `package.json`'s version field (frozen at 88.0.0) bumped to match.
- No function-level regression protection existed — every specific historical fix (clock-skew clamps, cross-category migration keying, etc.) was verified with a throwaway Playwright script, then deleted, leaving zero protection against silent reintroduction (`regress.js` only ever asserted "no thrown error," never "the right answer"). Added `scripts/unit-checks.js` — a permanent, real-browser (not stubbed-DOM) harness with 6 checks (skEffectiveLevel's clock-skew clamp and floor-at-1, skDaysLeft's floor-at-0, mergeNewSeedSkills' cross-category keying, integrityLevel's no-vows-yet and broken-keystone-cap behavior) — wired into `npm run verify`. CLAUDE.md now points future sessions at it instead of write-and-delete.
- `regress.js`'s skill audit extended to also assert `advance`/`maintain`/`roadmap` array lengths match `levels` — the exact defect class (blank ladder rungs, vanishing maintain guidance) that had needed two separate hand-run full-project audits (v212, v213) to find. A first pass also tried asserting every pyramid-tagged seed has an explicit `rarity` field; dropped after confirming it was a false-positive generator — `skRarity()`'s level-count fallback already resolves those skills to the correct tier in practice (verified against several flagged skills' actual `setKey` tier-letter prefix), so the check was flagging working skills, not real bugs.
- `dailies.js`'s `localYMD`/`todayStr`/`dayDiff` are called from earlier-loading files' top-level code purely because they're hoisted `function` declarations in one concatenated script — undocumented load-order fragility that a well-intentioned rewrite to `const x = (...)=>` would silently break at runtime with no syntax-check catching it. Added an explicit warning comment at the definition site.
- `habitStreakState`/`habitHeatMap`/`habitMonthGrid` (`dailies.js`) kept their pre-v168-merge names despite now operating on the unified Habits+Orders `S.dailies` array — renamed to `dailyTaskStreakState`/`dailyTaskHeatMap`/`dailyTaskMonthGrid`.
- `weight.js`'s `weightToast()` was a near-identical hand-copy of the app-wide `toast()` (same element, same show/hide choreography, different color/duration) — `toast()` now takes an optional `{color,duration}` second argument; `weightToast()` deleted, its 4 real call sites (2 in `weight.js`, 2 in `app-setup.js`) converted.
- `.gitignore` only covered `node_modules/`/`dist/`/`personal/` despite CLAUDE.md's own convention describing scratch `_*` files as git-ignored — they weren't. Added `/_*` (root-only, so it never matches `src/_shell.html`) and `scripts/_*`.

**Content — skill ladder honesty and completeness pass (785 skills total, both halves done by dedicated background agents given the scale):**
- 735 skills had `levels[0]` — the literal on-screen "promotion test" text for reaching Level 1 — open with a negative/failure-state description ("Can't define PMCS," "No written list, packs from memory") instead of a positive first action, a direct instance of CLAUDE.md's own rule #2 ("what the user can *do*, never a status label"). Rewritten to genuine, level-appropriate positive first actions consistent with each skill's own higher rungs — not templated, each independently authored from the skill's real name/why/howTo context. Plus 2 Personal/Roots capstone skills ("The Covenant," "The Living Root") whose ladder text used unfalsifiable trait-language ("you name the wrong thing in the room when it needs naming") instead of a real verification mechanism — rewritten to match the rigor their own Common-tier prerequisites (e.g. "Commitment log," which tracks real kept/missed rates over 10/25/50/90-day windows) already have. ("Sovereign Self," flagged for the same review, was inspected and left untouched — its ladder already bundles concrete quantifiers/artifacts and didn't meet the "genuinely unfalsifiable" bar.)
- Separately, `regress.js`'s new advance/maintain/roadmap-length audit found 71 real (non-false-positive) mismatches: 22 tactical Rare-tier skills (Combat Soldier / Field Operator / Leader's Tools / Intelligence / Specialties sets) were each missing exactly one trailing `maintain` entry for their peak level — fixed directly by hand, cross-checking each skill's actual final 1-2 levels against its existing `maintain` text first (caught and corrected 3 skills — Obstacle & Terrain, MDMP Execution, Troop Leading — where the existing entries were pre-existing off-by-one misalignments, not just short by one, from what looks like a level having been inserted earlier without the maintain array being resequenced). The other 48 (mostly Roots — meaning/gratitude/rest/relationships skills, plus a handful of physical/leadership/technical) had only 1 of 5 `maintain` entries — backfilled with 4 new entries each via a dedicated background agent, each genuinely specific to what that level's ability actually describes (not a copy-pasted template across all 48).

**Deliberately queued, not built** (both explicitly flagged during the audit as large enough to need their own dedicated session, the same treatment FM-1/FM-2/FM-3 got — Wyatt confirmed via `AskUserQuestion` to log only, not scope or build, this session):
- **A season/stage-aware Board tab.** Turning it from a static one-shot checklist into an MS1→MS4-aware "what does board/branch season need from me right now" view (the readiness rollup wired in this session is a smaller, separate fix — see above).
- **A `careerStage()`-driven career-arc layer.** `careerStage()` is currently used in exactly one real place beyond a one-time target backfill (skill-card importance ranking); using it to actually flavor FM programming phases (an LDAC prep block, a post-LDAC MS4 taper), milestone framing, and Board content is a deliberate multi-session training-program-design project, not UI wiring.
- Also noted but not attempted this pass, lower priority than the above two: a generic `wireEditableForm()` factory to collapse the 7 near-identical edit-form CRUD bodies (the shared cancel-link helper was already extracted in v213; the surrounding ~20-line pattern per form is still duplicated); a shared weekly-pattern-store factory for the near-identical gym-access/PT-day systems in `training.js`; splitting `app-setup.js` (6 unrelated subsystems — skill-form wiring, Records add-handlers, the Weight portal link, Awards/Membership/Event CRUD, cloud-sync, the TOC bridge — in one file with a misleadingly narrow name). All three are real but are pure refactors with no functional payoff on their own; better done in a session that already has a concrete reason to be in that code.

Verified via the new permanent `npm run unit` (6 checks, all passing) plus a simulated-old-save migration test confirming real progress (a skill at level 7/peak 7 with history) survives the `SKILL_LADDER_VER` bump and all 12,525 skills seed correctly (caught and fixed its own fixture bug along the way — a test skill missing an explicit `parent:null` was silently treated by `seedSkillsIfEmpty()` as a legacy pre-hierarchy flat save and wiped, not a real app bug). Full ship checklist: `npm run build` → OK (118,964 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525`, `npm run unit` → PASS. `SKILL_LADDER_VER` bumped 121→122 (785 skills' ladder text changed). SW bumped to `operations-v214`. `npm run package` → produced `dist/operations.zip`.

### v215 — season-aware Board tab

**Files changed:** `src/core/constants.js`, `src/core/migration.js`, `src/core/init.js`, `src/tabs/board.html`, `src/tabs/board.js`, `src/core/events.js`, `src/styles/main.css`, `scripts/unit-checks.js`.

Wyatt asked to build the first of the two items v214 had explicitly queued (not scoped or built). Used plan mode rather than building straight from the queue's framing: an Explore agent re-established the exact current state first — and corrected a v214 assumption in the process. `careerStage()` (migration.js — resolves `S.rank` free text to one of `MS1/MS2/MS3/LDAC/MS4/O1`) is **not** "used in exactly one real place beyond a one-time backfill" as v214's entry claimed; it's load-bearing across 7 sites in `skills.js` alone (card-rank ordering, the next-stage tick, the Gap Map table, the Assessment table, plus the two target-backfill sites in `migration.js`). That existing stage-target system (`SEED_SKILLS[].targets{}`, `updateAllSkillTargets()`'s explicit "sync to my career stage" button) became the direct pattern this feature reuses, rather than inventing a parallel one.

Two genuine product-behavior forks were resolved via `AskUserQuestion` before design: (1) new stage-appropriate tasks are added via an explicit **"↑ Sync tasks to my stage" button**, never silently — this exactly mirrors `updateAllSkillTargets()`'s `data-updateskilltgts` pattern; (2) past/future-stage tasks stay **visible, grouped into collapsed `<details>` sections** — only the current stage is expanded by default, nothing is ever hidden.

**What shipped:**
- `S.boardTasks[]` items gain `seedKey` (a stable content identity separate from the mutable `name`) and `stage` (`MS1–O1`, or `null` for user-added custom tasks, which always render in an always-visible "Your Tasks" bucket).
- New `BOARD_TASK_SEEDS` (constants.js) — 32 real, doctrine-grounded TBB-program tasks across the 6 stages: MS1 (early groundwork — meet cadre, start GPA logging, set a PT/AFT baseline, informally explore branches), MS2 (building the case — keep OML inputs current, narrow real branch choices, research DA PAM 600-3 requirements), MS3 (the original 10 tasks, reused verbatim — already well-covered), LDAC (deliberately the shortest list — Camp Summer Training isn't itself a board-prep activity, but Camp OML is a direct national-OML input), MS4 (post-board — confirm OML/branch result, process orders, finalize the accessions packet, research BOLC), O1 (commissioned — gaining unit, BOLC in-processing, ROTC out-processing). No invented specifics/dates/point values; hedging ("verify with cadre/current handbook/HRC") lives once, in the stage blurb + the existing bottom disclaimer, not repeated per task.
- New one-time migration `mergeBoardTaskSeeds()` (migration.js, gated on a new `S._boardTasksVer`, called from `init.js` right after `seedSkillsIfEmpty()` for the same load-order reason that function is) — retags **existing users'** already-saved copies of the original 10 tasks by matching their exact frozen wording, backfilling `seedKey`/`stage` without touching `id`/`done`/`due`. A task the user has since renamed simply won't match and lands in the ungrouped "Your Tasks" bucket instead of risking a wrong guess. Fresh installs ship pre-tagged in `DEFAULT.boardTasks` directly, needing no migration at all.
- `renderBoard()` (board.js) now groups tasks by stage into collapsed-by-default `<details class="wk">` blocks (current stage open) — reusing Plan tab's existing collapsible-card CSS (`.wk`/`.wk-day`/`.wk-body`/`.wk-target`) rather than inventing new visual language, per this project's design-system-reuse convention.
- New `syncBoardTasksToStage()` — adds any current-stage seed task not already present and not previously dismissed; wired via one new delegated line in `events.js` next to the `data-updateskilltgts` handler it mirrors.
- New `S.boardDismissedSeeds[]` (constants.js `DEFAULT`, plain `[]` — gets automatic old-save backfill for free via `state.js`'s existing array-coercion guard) — the delete handler records a deleted seeded task's `seedKey` here before removing it, so the sync button can never resurrect something the user deliberately removed.

Verified via 4 new permanent checks added to `scripts/unit-checks.js` (this project's established "don't verify with a script you then delete" pattern): migration preserves done-state and correctly retags, and is idempotent on a second call; sync adds the right tasks and is idempotent; a dismissed task never reappears on a later sync while its siblings do; fresh-install seeding is fully pre-tagged with an empty dismissal list. Plus a real behavioral smoke test at both 1200px and 390px viewport widths — confirmed a rank change correctly swaps which stage section is expanded, the sync button reports the right added-count and correctly says "already on your board" on a repeat click, and the collapsible-card layout reads correctly at phone width (screenshotted both). Full ship checklist: `npm run build` → OK (119,125 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525`, `npm run unit` → PASS. `SKILL_LADDER_VER` intentionally untouched (stays at 122 — this is a `boardTasks`-only migration, unrelated to skill ladders). SW bumped to `operations-v215`. `npm run package` → produced `dist/operations.zip`.

### v216 — career-arc layer (LDAC date, generalized taper, Dawn stage framing)

**Files changed:** `src/core/constants.js`, `src/tabs/profile.html`, `src/tabs/profile.js`, `src/core/training.js`, `src/tabs/today.js`, `src/tabs/board.js`, `scripts/unit-checks.js`.

The last item queued from v214's audit, originally framed as "an LDAC prep block, a post-LDAC MS4 taper, and milestone framing." Used plan mode again: two Explore agents surfaced a real constraint before any design — FM's entire training system (`SESSIONS`, constants.js) is built purely around the 5 official AFT events, with zero existing LDAC-specific content anywhere (no ruck programming, no land-nav conditioning, no O-course circuit), and real ruck-march loading carries genuine injury risk this app has no safety framework for yet (unlike its rappelling/grenades skills content, which does). Rather than build past that, asked via `AskUserQuestion` and got the safer, more scoped answer on both forks: no new FM exercise content (a real date field + generalizing the *existing* taper mechanism, nothing invented), and stay scoped to LDAC — no MS4-specific programming this session.

A Plan agent then resolved one real semantic question along the way: LDAC is a bounded, weeks-long event with only a start date captured, so its Dawn countdown deliberately does **not** mirror the commissioning banner's forever-after "past" branch (fine for an instant like commissioning; would read as "LDAC was 400 days ago" forever otherwise) — instead gated on `careerStage()==="LDAC"` staying true, going silent once the user's own rank text has moved past that stage.

**What shipped:**
- `S.profile.ldacDate` — new field, mirrors `commissionDate` exactly (same Profile input pattern, same save-handler line).
- `planForDay()`'s taper (`training.js`) generalized from AFT-test-date-only to the **nearest upcoming** of `{S.aftTestDate, S.profile.ldacDate}` — a date that's unset or already past is excluded from "nearest upcoming" via the exact same guard the original single-date version used. The taper label now names which event triggered it (`taperFor:"AFT"|"LDAC"`), flowing straight through to the existing session-label rendering with no separate UI change needed. `pickAftMode()` (which chooses an AFT-circuit day's specific mode: mock/practice/circuit) was deliberately left untouched — meaningless to generalize to LDAC, since an s4 day near LDAC should just ride the now-generalized intensity downgrade like any other hard session.
- New `ldacCountdownHtml()` (today.js) — extracted as a pure, directly-testable function (not inlined in `renderToday()`) with the three-state-plus-careerStage-gate logic described above. Reuses the existing `.commission-bar`/`.radiant` CSS as-is.
- New `stageContextHtml()` (today.js) — a small card surfacing the current stage's `STAGE_INFO` label/blurb (the exact same content Board already shows) as ambient Dawn context, not a second authored copy. `STAGE_ORDER`/`STAGE_INFO` relocated from board.js (where v215 built them, local to that file) to constants.js (shared, alongside `BOARD_TASK_SEEDS`) now that Dawn is a second consumer — confirmed via the build script's `JS_FILES` order that this is a pure, behavior-preserving move (both are `const`s referenced only inside function bodies that execute long after the whole script has parsed).
- Both new elements slot into the `flow` array right next to the existing `commissionHtml` — the countdown at the same urgency tier, the stage-context card in the ambient/informational cluster right after, consistent with the array's existing ordering discipline.

Verified via 6 new permanent checks in `scripts/unit-checks.js`: the nearest-of-two-dates taper selection in both directions (LDAC closer wins, AFT closer wins), a past-only LDAC date never taper, a past-only AFT date never tapers (this guard existed before but had zero regression coverage until now), neither-set is a true no-op, all 4 of `ldacCountdownHtml()`'s branches, and the `STAGE_INFO` relocation not breaking `renderBoard()`. Plus a real behavioral smoke test with screenshots confirming the Dawn banner and stage-context card render correctly together (LDAC set 5 days out, rank MS3) and Board still renders identically post-relocation. Full ship checklist: `npm run build` → OK (119,180 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525`, `npm run unit` → PASS. `SKILL_LADDER_VER` untouched (no skill/ladder content changed). SW bumped to `operations-v216`. `npm run package` → produced `dist/operations.zip`.

This closes out both items queued from v214's improvement audit — no backlog remains from that audit.

### v217 — FM coaching: injury-aware routing, proactive deload cadence, sleep-integrated recovery advisory

**Files changed:** `src/core/constants.js`, `src/core/training.js`, `src/core/state.js`, `src/tabs/plan.js`, `src/tabs/awards.js`, `src/tabs/profile.js`, `src/tabs/aft.js`, `src/tabs/today.js`, `src/tabs/test.js`, `scripts/unit-checks.js`.

Wyatt asked "is there anything else that can be done to improve FM and the coaching it can do." Rather than guess, dispatched 3 parallel research agents (periodization/macrocycle gap, sleep-to-recovery integration, injury-aware exercise routing) to confirm each was a real gap and scope the minimal honest fix before building anything — consistent with this project's standing research-before-design discipline. All 3 confirmed real; asked Wyatt for build order and he approved building all three.

**1. Injury-aware exercise routing.** No structured way existed to log a nagging injury and have exercise selection avoid it — Profile's free-text notes field was the only place, never parsed. The exercise library already tags every movement with muscle-group(s) (`SESSIONS[...].bw/gym/alt[].m[]`) and the equipment-eligibility filter already narrows each slot's pool using that exact tag-matching shape — so this reuses existing data and an existing filtering primitive rather than inventing a new classification scheme. New `S.avoidTags[]` (constants.js `DEFAULT`), set via a new "Working Around Something?" card (`renderAvoidTagsUI()`, plan.js, in the same Setup section as Equipment Profiles) listing every muscle tag actually in use — derived live via a new `allMuscleTags()` (training.js) rather than hand-maintained, so it can't drift from the real exercise data. New `applyAvoidTags(pool, avoidTags)` (training.js) is a **soft** preference, not a hard exclusion: narrows a slot's pool to genuinely unflagged alternatives, but falls back to the full pool rather than ever leaving a slot with nothing to suggest — the research flagged that some slots (especially the Run/AFT-circuit alt pools) skew heavily toward `legs`/`cardio` tags, so an aggressive hard filter could gut a whole session with no graceful fallback. Wired into `sessionExForProfile()`'s `resolveSlot()` in place of the old equipment-only filter.

**2. Proactive deload cadence.** FM's only two intensity-reducing mechanisms were both reactive: a taper right before a declared AFT/LDAC date, and an overtraining advisory that only fires after 21 days of already-manifested struggle signals. Nothing scheduled a lighter week in advance — absent an imminent test date or a struggle streak, `computeTarget()`'s default is to keep escalating load indefinitely. (The research surfaced a nice piece of self-corroborating evidence: the Skills tree's own Physical-path content, entirely separate from the coach engine, already teaches cadets that scheduled deloads "before fatigue arrives" are correct practice — the scheduler just never followed its own app's advice.) Fix: extracted `pickRunIndex()`'s epoch-anchored continuous week counter into a shared `weeksSinceEpoch()` (training.js, zero behavior change to the run rotation itself) and added a `week%4===3` deload check to `planForDay()`, reusing the exact same hard→moderate downgrade shape the taper already uses. No new `S` field — reuses a clock the app already trusted. Taper always wins when both would apply on the same day (test/report-date proximity is more specific and urgent than routine periodization); the deload check is explicitly gated on `!plan.taper`.

**3. Sleep-integrated recovery advisory.** Confirmed completely disconnected before this: `recoveryReadiness()` only ever read resting-HR and HRV against baseline; the deload advisory's own copy tells the user to "prioritize sleep" while using zero sleep data to detect the need for it. The real complication (why this hadn't been built already): Apple Health sleep records are `HKCategoryTypeIdentifierSleepAnalysis` — an INTERVAL (start→end), not a point value like every other imported metric, with a string sleep-stage `value` (`AsleepCore`/`AsleepDeep`/`AsleepREM`/`AsleepUnspecified`, vs. `Awake`/`InBed` which are excluded) instead of a number, so it silently fell through the existing point-value parser's own `parseFloat`/`isNaN` guard. Added a dedicated interval-collecting branch to the streaming Apple Health parser (`parseAppleHealth()`/`handleChunk()`, profile.js) bounded to the last 35 days (matching `healthImport.history`'s existing 30-day cap — even a multi-year export's full interval list would be memory-cheap, but there's no reason to keep years of it). New `bucketSleepIntervals()` groups raw intervals into per-night totals: buckets a session starting before 14:00 local to the *previous* night (covers sleep continuing past midnight) and merges overlapping intervals within a night before summing (multiple sources — e.g. Watch + iPhone — can each log their own overlapping stage records for the same sleep; naive summing would double-count that overlap). Bucketed nightly hours merge into `healthImport.history` alongside rhr/hrv/vo2 (by date, `Object.assign`-merged so it can't clobber an existing entry). `recoveryReadiness()` gained a sleep flag using a **fixed 7-9h target**, deliberately not a self-relative baseline like rhr/hrv use — "1h below your own average" doesn't mean the same thing for someone who normally gets 5h vs. 8h, so a self-relative comparison would be less honest here, not more.

**A real, unrelated bug found and fixed along the way:** while writing `ldacCountdownHtml()`'s own unit test, discovered its "today" branch (and the identically-patterned pre-existing `commissionHtml` it was modeled after in v216) mislabeled the actual day-of as "1 day away" whenever checked before noon local time — `Math.ceil()` on a noon-anchored millisecond diff against `Date.now()` rounds any same-day-but-still-positive gap up to 1, not 0. Swept the whole codebase for the exact same pattern and found **11 total sites** carrying it: `today.js` (LDAC/commission banners, the Upcoming timeline's day labels, `copyDailyBrief()`'s commission line, overdue counseling follow-ups, blood-donation eligibility — the last of these also had a second, separate UTC-vs-local date-parsing bug stacked on top), `state.js` (boss pace-to-target), `aft.js` (AFT test countdown), `plan.js` (weeks-to-test recommendation), `profile.js` (milestone "in Nd"/"Nd ago" labels), `test.js` (study-plan days-left). All fixed with the same time-of-day-invariant pattern already proven safe elsewhere in the codebase: `dayDiff()` on calendar-date strings (`localYMD()` vs. the stored date string) instead of a millisecond diff against the live clock.

Verified via 8 new permanent checks in `scripts/unit-checks.js`: `applyAvoidTags()` narrows the pool when a safe alternative exists, falls back to the full pool rather than emptying a slot, and leaves the pool untouched when nothing's flagged; the deload cadence never lets a hard day survive an unqualified deload week and actually fires at least once across a 60-day scan; taper and deload are never both true on the same day; `bucketSleepIntervals()` correctly merges overlapping intervals (7.5h, not 11.5h double-counted) and correctly buckets a post-midnight session to the previous night; `recoveryReadiness()`'s new sleep flag fires correctly on a short night. Plus a real behavioral smoke test (screenshotted) confirming the avoid-tags UI renders and actually changes exercise selection, a real upcoming deload week resolves and labels correctly, and a simulated short-sleep night produces the right recovery-caution message. Full ship checklist: `npm run build` → OK (119,351 lines), `npm run check` → SYNTAX OK, `npm run regress` → `PAGEERRORS 0`, `badCount:0`, `total:12525`, `npm run unit` → PASS. `SKILL_LADDER_VER` untouched (no skill/ladder content changed). SW bumped to `operations-v217`. `npm run package` → produced `dist/operations.zip`.
