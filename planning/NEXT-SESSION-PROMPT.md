...
**Current version: v158.** The service worker is at `operations-v158` in `sw.js`. `SKILL_LADDER_VER` is currently **117** (in `src/core/migration.js` — unchanged since v155; v156 through v158 didn't touch it). Total skills: **6899** (6274 + 625 from v158's new tree).

**v157 found a real error in the previous version of this file** — read it before trusting any "N trees remain" claim, including this one, without re-deriving it yourself. See "Pyramid state" below. v158 re-derived the count fresh at session start and confirmed it matched exactly (no drift) — still worth re-deriving yourself every session rather than trusting this number once enough time has passed.
...
- **Next: the Commons layer for the remaining 9 Mythic trees** (5,625 cards) — seven trees down, one proven-working process, repeat it — see below
...
The prerequisite is done, the object shape is established, and seven trees (Physical Mastery + Keeper of the Flame + Tactical Mastery + Battlefield Commander + Vital Operator + Scholar-Warrior + Soldier Athlete, 4375 skills) are written and verified as of v158. What's left is repeating that process for the other 9 Mythic trees: Cyberspace Operations Officer, Master of the Mind, Sovereign Self, The Living Root (legacy-gen), and Warrior Foundation, Staff Excellence, Cyber Operator, Cognitive Athlete, Life Architect (second-gen Phase 7).

1. **Confirm the current total** before starting, in case anything has drifted. Don't just grep for a fixed expected number — actually walk all 16 Mythic trees down to their Commons layer with a Node script (see the v157/v158 `FINISHED-FEATURES.md` entries for the exact approach) and see which trees already have 625 Commons vs. which have 0. Total `SEED_SKILLS` should read **6899**. If it's off, or if a tree you expect to be untouched already has Commons (or vice versa), something changed and needs auditing the same way v148-v158 did.
2. **Pick the next Mythic tree** to complete from the 9 remaining. Check for name+cat collision risk in that `cat` first via the duplicate sweep in the Ship checklist; `technical`/`personal`/`cognitive` carry the 10 known pre-existing collisions, so a first pass there needs extra care. Each tree is 125 setKeys / 625 skills, same shape as v150–v158.
3. **Reuse the v150–v158 Common object shape exactly** — no redesign needed: `rarity:"common"`, `fadeDays:30`, `setKey` = the parent Uncommon's own `synthesizedFrom` value, 4-item `levels`/`roadmap`/`advance`/`maintain` arrays, no `synthesizedFrom`/`unlockHint`/`tiers` fields. Grep any `phys_c_*`, `hearth_c_*`, `tac_c_*`, `lead_c_*`, `phys2_c_*`, `acad_c_*`, or `physb_c_*` skill for a concrete reference.
4. Write `planning/IMPROVEMENTS-vNNN.md` scoping the chosen tree, then execute with the SLOT-marker + parallel-subagent pattern (see "Writing seeds at scale" below) — one agent per parent Rare (5 setKeys/25 skills per agent). **Cap concurrency at 2 agents per wave, not more** — v154 reduced from 4 to 2 after two file-corruption incidents happened at 4-agent concurrency, and v155–v158 all confirmed zero incidents at 2-agent concurrency across four more full trees. Don't scale back up.
...
# bump sw.js: operations-v158 -> operations-v159 (or whatever's next)
...
- Don't scale concurrency back up past 2 agents/wave — v154 through v158 collectively represent 5+ full trees written at 2-agent concurrency with zero file-corruption incidents; 4-agent concurrency caused two in a single v154 wave.
