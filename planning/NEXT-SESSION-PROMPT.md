...
**Current version: v205.** The service worker is at `operations-v205` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v205 touched no ladder/tier/guidance content). Total skills: **12524**.
...
**FM roadmap — all 4 confirmed items now shipped (v204/v205):**
1. ✅ **v204 — baseline-test warm-up safety protocol + true beginner on-ramp.** `BASELINE_TEST` entries (`training.js`) carry `warmupProtocol`/`warmupNote`; `SESSIONS` (`constants.js`) gained "Knee push-ups" and "Run-walk build-up (beginner)" as swappable `alt` pool entries with real `BEGINNER_RX` rows and `EX_HOWTO` guidance.
2. ✅ **v205 — macro-level deload/overtraining detection.** New `detectOvertrainingTrend()` (`log.js`) + `renderDeloadAdvisory()` (`plan.js`) — reads real struggle signals across the last 21 days of `S.workouts`, fires only on a genuine ≥50%-of-sessions pattern, not one hard day.
3. ✅ **v205 — real AFT taper window.** `pickAftMode()` no longer allows a full mock AFT in the final 6 days before a declared test (caps at lighter single-event `"practice"`); `planForDay()` downgrades hard-intensity sessions to moderate in that same window.
4. ✅ **v205 — honest nutrition/protein guidance.** New `renderNutritionCard()` (`plan.js`), a collapsed "🍗 Nutrition basics" section in the Coach Hub — real protein range from logged bodyweight when known, qualitative (not fabricated-precision) calorie framing, same "informational, not medical advice" framing as the Vitals section.

**Not yet scoped with Wyatt — lower-priority/more sensitive FM items, ask before building:** the SDC-specificity gap (weakest AFT event for training volume/frequency) and connecting the Army height/weight standard (AR 600-9) to the existing Weight tab (audit flagged this as real but personal enough to raise explicitly, not assume).
