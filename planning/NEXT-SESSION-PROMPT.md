...
**Current version: v202.** The service worker is at `operations-v202` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v202 touched no ladder/tier/guidance content). Total skills: **12524**. **v199: the Today tab's compact session preview (`dawnSessionHtml()`, plan.js) now shows real per-exercise targets via the same `computeTarget()` call Coach Today makes — was names-only before.** This closes the "make everything use coach" ask from v197/v198. **v200 fixed 2 small live bugs** (Board's `grant()` used an invalid `"knowledge"` path key instead of `"academic"`; Battle Buddy Report read `a.n`/`S.events.hours` instead of the real `a.title`/`S.volunteer`). **v201 fixed the broken Path-development math** (see the v201 `FINISHED-FEATURES.md` entry — `catRolledLevel()` deleted entirely, `catProgressFraction()` as a % is now the one canonical Path-development number everywhere; Garden's idol growth now credits the leveled skill's own Path instead of always Academic) **and found + fixed the real root cause of the long-flagged v195 "cosmetic NaN" warning** (a copy-paste seed-hash bitmask typo in `skEmblemSvg`'s `_physical()` sigil generator, `skills.js`). **v202 fixed the 2 permanently-stuck-at-0 skills** — built a real 10th stealth-assessment game (Signal Intercept, `test.js`) for Pattern recognition, and wired Memory retention to real SRS ease-factor stats (`auto-level.js`'s new `memRetentionLevel()`).

**One more FM-redesign-sized workstream is still queued, found by the v200-session 3-agent audit and confirmed wanted by Wyatt ("please do all of these") — read the v200/v201/v202 `FINISHED-FEATURES.md` entries before starting it:**
1. **Five competing "what skill needs attention" systems on the Today/Dawn tab** (`getWarriorsFocus()`'s decay step, Today's Hand random draw, `computeSmartFocus()`'s callout, the "Skill of the day" block, and the "Fading soon" digest) can each recommend a different skill simultaneously, computed via 3 different independent urgency thresholds. `computeSmartFocus()` is the newest/most deliberate and should likely become the one canonical ranking, with Today's Hand kept as the one intentionally-different "practice, not urgency" mode.

**One small loose end from the v202 auto-skills fix, not done and not blocking:** the audit also flagged that the "remember things" concept is split across 4 skills (`Memory span`, `Memory technique`, `Memory retention`, `Study & retention`) with no shared model. v202 only fixed `Memory retention`'s dead auto-lock (it now levels from real SRS ease-factor stats, capped at L5 — see the v202 entry); it did NOT do the broader pass of reconciling all 4 skills' charters so they measure genuinely distinct things. Pick up if it comes up again, not urgent.
