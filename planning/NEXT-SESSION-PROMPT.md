...
**Current version: v204.** The service worker is at `operations-v204` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v204 touched no ladder/tier/guidance content). Total skills: **12524**.
...
**FM roadmap (Wyatt confirmed start order via `AskUserQuestion`):**
1. ✅ **v204 — baseline-test warm-up safety protocol + true beginner on-ramp.** See the v204 `FINISHED-FEATURES.md` entry. `BASELINE_TEST` entries (`training.js`) now carry `warmupProtocol`/`warmupNote`; `SESSIONS` (`constants.js`) gained "Knee push-ups" and "Run-walk build-up (beginner)" as swappable `alt` pool entries with real `BEGINNER_RX` rows and `EX_HOWTO` guidance.
2. **Not started — macro-level deload/overtraining detection.** Every existing recovery mechanism (`recoveryLoad()`, `spaceOutHardDays()`, `computeTarget()`'s per-exercise hold-on-hard-effort) operates at a 4-day-window or single-exercise level. Nothing looks at 2-4 weeks of trend (repeated "cut short" flags, declining RPE tolerance across multiple exercises) and calls for a real whole-week volume-reduction. Real gap, high leverage.
3. **Not started — taper window before a declared AFT test date.** `pickAftMode()` (`training.js`) currently *increases* mock-AFT frequency in the final 14 days before a real test with nothing backing off elsewhere — the opposite of standard taper practice.
4. **Not started, confirmed wanted — honest nutrition/protein guidance** tied to training load. Frame as educational, non-medical-advice, matching the app's existing convention (see Weight tab, vitals sections).
5. Lower-priority/more sensitive, not yet scoped with Wyatt: SDC-specific conditioning gap (weakest AFT event for volume/frequency), and connecting the Army height/weight standard (AR 600-9) to the existing Weight tab — audit flagged this as real but personal enough to ask about explicitly before building, not assumed.

**Quiz roadmap (Wyatt confirmed start order via `AskUserQuestion`):**
1. **Not started — SRS auto-feed mechanism.** Wrong quiz answers (`answerQuiz()`, `quizzes.js`) never become spaced-repetition cards; `S.srsDecks` is 100% manually authored. This is the single biggest mechanism gap everything else compounds on.
2. **Not started — highest-impact missing content categories** in `quizbank.js` (currently 16 categories, 124 questions total): rank/insignia (zero coverage), radio comms/prowords/9-line/SALUTE report (the "Communication" category is actually generic interpersonal theory, not radio procedure — misleading name), drill commands, AFT-standards knowledge (distinct from the app's AFT score-entry tool — this would test *knowledge of* the standards).
3. Later, not yet started: expand thin categories (leadership: 6 questions, profession: 5, history: 6 — Revolutionary War only), fix the "Weapons" category (promises M9/M17 in its name, has zero pistol questions), add per-question weak-area tracking (currently only per-category `{passed,bestPct,attempts}`), add a real board-readiness view.

Read both full audit reports (dispatched as background agents in the v204-session conversation) before continuing this workstream if the summary above isn't enough context — they have exact evidence (file/line citations, exact category names/counts) this summary compresses.
