...
**Current version: v200.** The service worker is at `operations-v200` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v200 touched no ladder/tier/guidance content). Total skills: **12524**. **v199: the Today tab's compact session preview (`dawnSessionHtml()`, plan.js) now shows real per-exercise targets via the same `computeTarget()` call Coach Today makes — was names-only before.** This closes the "make everything use coach" ask from v197/v198. **v200 fixed 2 small live bugs** (Board's `grant()` used an invalid `"knowledge"` path key instead of `"academic"`; Battle Buddy Report read `a.n`/`S.events.hours` instead of the real `a.title`/`S.volunteer`).

**Three FM-redesign-sized workstreams are now queued, found by a 3-agent audit in the v200 session and all explicitly confirmed wanted by Wyatt ("please do all of these") — read the v200-session audit findings (not yet written to FINISHED-FEATURES.md as their own entries; each gets one when it ships) before starting any of them:**
1. **Path-development math is broken.** `catRolledLevel()` (skills-core.js) averages a Path's level across every "top-level" skill, but the Commons-layer work (v150–v167) added ~10,000 flat per-Path skills never wired into that average correctly (they use `setKey`, not `parent`). Result: `catRolledLevel` now computes a permanent near-zero (e.g. "Lv 0.1") regardless of real progress, and it's still shown in the Tree world label, both corners of every Skills-tab deck header, and Plan's "Overall physical level" line. `catProgressFraction()` (added v172) is the correct, already-working replacement but is only wired into the Tree's glow. Separately, the Garden's idol growth runs on a disconnected `S.pathXP` counter that 6 of 10 Paths barely ever receive from real skill leveling, contradicting the Grove's own "every deed feeds the idol" copy. Needs a design decision with Wyatt on: which metric becomes canonical everywhere, and what `pathXP`/Garden growth should actually mean.
2. **Two skills are permanently stuck at level 0.** `Pattern recognition` (`auto:"test:patterns"`) and `Memory retention` (`auto:"quiz:retention"`) are locked as "auto-leveling" but nothing in `auto-level.js`/`test.js` actually levels either — no `"patterns"` test exists, no `"quiz:retention"` handler exists. `Memory retention`'s own `howTo` copy directly contradicts its lock ("there is no automatic leveling for this skill yet — honesty here is the whole point"). Broader: the "remember things" concept is split across 4 skills (`Memory span`, `Memory technique`, `Memory retention`, `Study & retention`) with no shared model. Needs a design decision: build a 10th Phase-T-style stealth game for Pattern recognition (or un-auto it), and wire `Memory retention` to real SRS stats from `S.srsDecks` (or un-auto it) — plus reconciling the 4-skill cluster's charters.
3. **Five competing "what skill needs attention" systems on the Today/Dawn tab** (`getWarriorsFocus()`'s decay step, Today's Hand random draw, `computeSmartFocus()`'s callout, the "Skill of the day" block, and the "Fading soon" digest) can each recommend a different skill simultaneously, computed via 3 different independent urgency thresholds. `computeSmartFocus()` is the newest/most deliberate and should likely become the one canonical ranking, with Today's Hand kept as the one intentionally-different "practice, not urgency" mode.
