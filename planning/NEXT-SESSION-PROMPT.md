...
**Current version: v190.** The service worker is at `operations-v190` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v190 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)
...
**v190 also shipped Phase A of the GUI revamp** (font self-hosting + a design-system foundation + Plan/Log/AFT structural restructure + nav reorg) — see the dedicated section below and the v190 `FINISHED-FEATURES.md` entry for full detail. **Phase B (migrating the other ~15 tabs, deciding the Weight tab's identity, deeper drawer interaction changes) is explicitly not scoped yet** — don't assume it's next.

There is no queued next phase from the FM/test-features doc, and GUI revamp Phase B needs its own scoping pass, not silent continuation. The next session should open by asking Wyatt what he wants to work on next.
...
- **Item 2 (nav restructuring) + item 3 (mobile): DONE, v174 + v190.** v174 built the mobile bottom-bar (4 tabs) + slide-up drawer (the other 14), replacing an unpaginated horizontal-scroll strip. v190 did the desktop side v174 explicitly left alone: replaced the old 7-primary/11-"More" split's zero-documented-rationale flat list with real category group headers (Testing & Study / Identity & Record / Objectives / Reflection & Reward), applied the same grouping to the mobile drawer plus a pinned "Training" group (Plan/AFT/Log, which are desktop-primary but mobile-secondary) first. Desktop-7 and mobile-4 deliberately keep different counts now, with a stated rationale (every-session vs. every-day-in-seconds) instead of silence. **Any further nav change (e.g. actually changing which specific tabs are primary, or a deeper drawer interaction model like collapsible sections) is new scope, not implied by this.**
- **Item 1 (visual/theme refresh): PHASE A DONE, v190; PHASE B NOT SCOPED.** v174 was explicitly "light polish only, don't redesign." v190 went further — Wyatt confirmed "open to a fuller creative redesign" — but Phase A deliberately built the *foundation* (design tokens, a shared button/card class vocabulary, a 197-occurrence rgba→var() mechanical sweep, dead-font cleanup, self-hosted fonts) and applied it to Plan/Log/AFT specifically, rather than touching all 45+ button classes / 40+ card classes / 18 tabs in one pass. **This is a real, currently-true state: most of the app's 18 tabs (15 of them) still use the pre-revamp hand-declared button/card CSS** — that's Phase B, explicitly deferred, not forgotten. See the v190 `FINISHED-FEATURES.md` entry for the full token/class API and the exact list of what Phase B still covers.

**Still open:**
1. **Phase B — migrate the other ~15 tabs** onto the `.btn`/`.card` system Phase A established, tab by tab or in a batch — needs its own scoping pass (which tabs first, how much visual change beyond the token migration itself) before starting, per the standing "ask before large shifts" rule.
2. **The Weight tab's identity** (serif font + a separate near-duplicate tan palette, confirmed to live in one deliberate "set apart" CSS block) — decide intentionally whether it's a keeper or gets unified with the rest of the app. Not yet asked.
3. **Deeper drawer interaction changes** (e.g. collapsible per-category sections in the nav drawer) — v190 only added static group-header labels; a real accordion/collapse model would be new scope.
4. Font self-hosting is now DONE (v190) — no longer open. `fonts/oswald.woff2`/`fonts/roboto-condensed.woff2` are vendored and precached.
