...
**Current version: v189.** The service worker is at `operations-v189` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187/v188/v189 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)

**The entire FM/test-features workstream (`planning/IDEAS-tests-fm-workouts.md`) is now DONE — X-Timeline, X-AAR, FM-1, FM-2, FM-Adapt, X-Insight, Phase T (all 9 constructs + the focus-picker menu), FM-3, and X-SmartFocus have all shipped (v175 through v189).** There is no queued next phase from that doc. The next session should open by asking Wyatt what he wants to work on next — don't assume a workstream; the last several sessions all had a standing confirmed build order to fall back on, and that's no longer true.
