...
- **NEW phase, raised live during FM-2, not yet designed: FM-Adapt** — Wyatt asked for the app to learn from logged difficulty ratings (pre-set, post-set, and a mid-set "had to reduce" flag) and use that history to adjust future rep/set/weight suggestions per exercise, continuously. This is genuinely large and undesigned — same caliber as X-Insight below, and shares a mechanism with FM-3's already-designed (unbuilt) card-game adaptive-bias (§5a point 4). **Needs its own dedicated design pass before any code** — see the FM-Adapt entry in `IDEAS-tests-fm-workouts.md` for the full writeup, including exactly what's already shipped (the non-adaptive weight suggestion) vs. what's still undesigned.
- **Next phase: FM-Adapt's design pass**, or if Wyatt wants to skip ahead, **X-Insight** — see `IDEAS-tests-fm-workouts.md` for full scope/dependencies on each.
