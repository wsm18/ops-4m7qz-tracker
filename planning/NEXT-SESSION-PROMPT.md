...
**Current version: v192.** The service worker is at `operations-v192` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v192 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)
...
`planning/IDEAS-tests-fm-workouts.md` is fully complete: X-Timeline, X-AAR, FM-1, FM-2, FM-Adapt, X-Insight, Phase T (all 9 stealth-assessment constructs + the focus-picker menu), FM-3, X-SmartFocus (v175–v189). No queued next phase — ask Wyatt what to work on next. Full detail is in each version's `FINISHED-FEATURES.md` entry; the load-bearing gotchas if you touch this code again:
