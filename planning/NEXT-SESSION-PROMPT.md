...
**Current version: v195.** The service worker is at `operations-v195` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v195 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)
