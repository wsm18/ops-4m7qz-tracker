...
3. `planning/IDEAS-tests-fm-workouts.md` — **the active workstream as of v187**, confirmed build order at the bottom. Read the "Decisions needed"/"Recommended build order" sections before picking the next phase.
...
**Current version: v187.** The service worker is at `operations-v187` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)
...
**Phase T (stealth-assessment games) is now fully DONE as of v187 — all 9 constructs shipped.** Every `test.js` cognitive drill and the `quizzes.js` knowledge quiz now has a real game skin (Sentry, Land Nav Relay, Comms Relay, Perimeter Watch, Fire Discipline, Cipher Desk, Fire Mission, Intel Briefing, Climb the Tree) — see the v187 `FINISHED-FEATURES.md` entry for what each does and two real bugs it caught (a repeat-waypoint tap bug and a click-event-bubbling round-skip bug, both in Land Nav Relay, both fixed). One piece from the original idea #1 scope is still open and unbuilt: **the session-start "pick a focus" menu** (present what-to-train-today instead of a flat test list) — now's the natural time to build it, since every construct is finally converted.
...
## FM/test-features workstream — active as of v187, 7 of 9 top-level phases fully done (Phase T just finished)
...
- **Phase T (stealth-assessment games) — DONE, v186 (Sentry) + v187 (the other 8).** All 9 constructs shipped: Sentry (reaction), Land Nav Relay (digit span), Comms Relay (typing), Perimeter Watch (n-back), Fire Discipline (go/no-go), Cipher Desk (processing speed), Fire Mission (mental math), Intel Briefing (reading — a real measurement upgrade, objective comprehension replacing self-report), Climb the Tree (quizzes — first-attempt-accuracy scoring with unlimited retries). See the v186/v187 `FINISHED-FEATURES.md` entries — v187's includes two real bugs a throwaway Playwright playthrough caught (repeat-waypoint tap bug, click-bubbling round-skip), both in Land Nav Relay, both fixed. **Still open from the original idea #1 scope:** the session-start "pick a focus" menu (present what-to-train-today instead of a flat test list) — now's the right time to build it, since every construct is finally game-skinned rather than mixed old/new.
- After Phase T: FM-3 (card-game workouts) and X-SmartFocus remain; both need their own greenlight/design pass respectively before building (see their entries).
- Remaining phases: FM-3, X-SmartFocus — see `IDEAS-tests-fm-workouts.md` for full scope/dependencies on each. **FM-3 (card-game workouts) is explicitly not build-greenlit even though its design is complete — Wyatt asked to hold it in planning; re-confirm before starting that one specifically, whenever its turn comes up.**
