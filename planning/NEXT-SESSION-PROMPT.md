...
**Current version: v193.** The service worker is at `operations-v193` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v193 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)
...
**v193, immediately after, was a docs-cleanup follow-through plus two real fixes Wyatt flagged live.** Deleted `IDEAS-gui-revamp.md`, `IDEAS-tests-fm-workouts.md`, and `docs/operations-expansion-outline.md` (all fully superseded, zero open items — see the v193 `FINISHED-FEATURES.md` entry), rewrote the badly-stale `docs/OPERATIONS-HANDOFF.md`, and refreshed `README.md`/`docs/SETUP.md`. **Real bug:** the Adaptive Targets card (`renderAdaptiveTargets()`, `log.js`) — the "🎯 Your Next-Session Targets" trainer — was accidentally made hard to find in v190's Plan-tab restructure, mounted inside a collapsed-by-default "⚙️ Setup" accordion alongside static equipment/weather settings. Moved to sit always-visible below Coach Today. **Real gap:** `computeTarget()` never read `S.aft` (real AFT test history) at all — now blends the 4 AFT events with a clean logged-exercise match (deadlift, push-ups, plank, the 2-mile run — **not** SDC, no clean match exists) as a corroborating signal exactly like the existing baseline-test blending, and seeds a starting target from real AFT history when there's no workout-log entry yet for that exercise (previously went silent). **If you touch `computeTarget()` or the Plan tab's top-of-tab card order again, read the v193 entry first** — both fixes are grounded in exact function/line-level detail there.
