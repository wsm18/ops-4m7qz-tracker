...
3. `planning/IDEAS-tests-fm-workouts.md` — **the active workstream as of v188**, confirmed build order at the bottom. Read the "Decisions needed"/"Recommended build order" sections before picking the next phase.
...
**Current version: v188.** The service worker is at `operations-v188` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — neither v187 nor v188 touched ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)
...
## FM/test-features workstream — active as of v188, 8 of 9 top-level phases fully done (only X-SmartFocus remains)
...
- **Remaining phase: X-SmartFocus** — the last item in the doc, needs its own goal-weighting design pass before it's buildable. See its entry in `IDEAS-tests-fm-workouts.md` for full scope.
