...
**Current version: v194.** The service worker is at `operations-v194` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v194 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)

**v192 was a full project cleanup, not a new feature** — dead code removal (a real `tree.js` event-listener leak, dead functions, ~30 orphaned CSS blocks) plus real bugs found along the way (a name-matching bug in FM-3's `cgFindRxRow()`, the Plan tab rx-card showing the wrong exercise on a swap day, missing recovery-day spacing in `assignWeekSessions()`, Intel Briefing having zero real CSS since it shipped, a dead CSS selector) and planning-doc maintenance (`SESSION-TIMES.md` reordered to match its own stated convention, `FINISHED-FEATURES.md` condensed + given a version index, `NEXT-SESSION-PROMPT.md` trimmed). See the v192 `FINISHED-FEATURES.md` entry for the full list before assuming any of those areas still have the old bugs.

**v193 was a docs-cleanup follow-through plus two real fixes Wyatt flagged live** (deleted 3 fully-superseded planning docs, rewrote the badly-stale `docs/OPERATIONS-HANDOFF.md`, and wired `S.aft` history into `computeTarget()` for the first time). **v194, same session, went further — read this one, not v193, for the Plan tab's current shape:** the standalone "Adaptive Targets" card v193 had just made visible **no longer exists at all** — its content is now merged directly into whatever you're already looking at, per Wyatt's explicit ask ("the coaching/recommendations should replace the previous FM stuff... instead of needing to find it separately"). `renderSessionLists()`'s Session N reference cards now show real `computeTarget()` output (the same engine Coach Today already used) instead of the old static `BEGINNER_RX` beginner numbers, whenever real log/AFT history exists — falling back to the beginner row only for an exercise with none yet. The Session block matching **today's actual scheduled session** now opens automatically (`openTodaysSessionBlock()`, called once on nav-to-Plan) instead of Session 1 always being hardcoded open regardless of the real schedule. **If you touch `computeTarget()`, `renderSessionLists()`, or the Plan tab's card order/layout again, read the v194 `FINISHED-FEATURES.md` entry first** — it has the full before/after and exactly why the standalone card was removed rather than kept alongside the merge.
