...
**Current version: v208.** The service worker is at `operations-v208` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v208 touched no ladder/tier/guidance content). Total skills: **12524**.
...
**FM+Quiz quality workstream (v204-v208) — FULLY COMPLETE.** Wyatt's framing: FM is the priority ("I am currently really bad at" physical training, wants real help), quizzes should "prepare me for anything in that topic I could encounter." Two deep research audits (dispatched as background agents in the v204-session conversation — read them directly if you need the original exact evidence/citations) drove every item below; all confirmed findings are shipped. See each version's own `FINISHED-FEATURES.md` entry for full detail:
...
- **Quiz (v206/v208):** SRS auto-feed (wrong answers → spaced-repetition cards automatically), 4 new content categories (rank/insignia, radio comms/prowords/9-line/SALUTE, drill, AFT-standards knowledge — bank is now 20 categories), 3 thin categories expanded (leadership/profession/history), the Weapons category's name-vs-content mismatch fixed (added real M17/M9 pistol content), and a real board-readiness view (`renderBoardReadiness()`, `quizzes.js`) that surfaces per-category tracked weak spots from the SRS miss-deck data instead of a flat pass-count tally.

**Current ask (given, not yet executed as of v208): a second, deeper audit pass on FM/Quizzes, AND a genuinely comprehensive audit of the ENTIRE project** ("absolutely everything should be looked at"). This is broader than the v200-session 3-agent audit (which covered skills/pyramid, test/quizzes/records, and the daily-loop tabs) — scope it to cover whatever that one didn't reach in depth, plus anything cross-cutting (architecture/tech-debt, performance, cross-tab consistency) neither prior audit round touched. Read this conversation's history for the exact framing before scoping the audit dispatch.
