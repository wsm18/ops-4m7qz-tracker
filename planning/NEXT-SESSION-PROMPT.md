...
**Current version: v197.** The service worker is at `operations-v197` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v197 touched no ladder/tier/guidance content). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)

**v197 fixed two real bugs stopping "Today's Orders" (`renderCoachToday()`'s heading, `#coachToday` on the Coach Hub) from showing real adaptive numbers — read the v197 `FINISHED-FEATURES.md` entry before touching `computeTargetFallback()` again.** (1) Logged workout exercise names transcribed in v193/v196 didn't exactly match `SESSIONS`' canonical names, so tier-1 "adaptive" resolution silently missed real history — fixed by renaming 16 logged exercise names to their canonical equivalents, pushed live via TOC's own `POST /api/projects/operations/data`. (2) `computeTargetFallback()`'s tier-3 "starter" branch only checked one `BEGINNER_RX` list (`gym` or `bw`) based on the day's gym-access flag, missing bodyweight-only accessories that appear even on gym days — fixed to check both, which then surfaced a second bug: the word-overlap matcher started false-matching warm-up/cool-down stretches onto unrelated strength rows via common short words like "hold". Fixed by gating the whole tier-3 branch on `exArg._phase` not being warmup/cooldown/flex.
