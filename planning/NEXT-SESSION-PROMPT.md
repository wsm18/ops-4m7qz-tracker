...
3. `planning/IDEAS-tests-fm-workouts.md` — **the active workstream as of v177**, fully designed, confirmed 8-phase build order at the bottom. Read the "Decisions needed"/"Recommended build order" sections before picking the next phase.
...
**Current version: v177.** The service worker is at `operations-v177` in `sw.js`. `SKILL_LADDER_VER` is **117** (unchanged since v155). Total skills: **12524**.
...
## FM/test-features workstream — active as of v177, 2 of 8 phases done
...
- **Next phase: FM-1** (gym-schedule-aware planning + guided mock-AFT flow, ideas #2/#2b) — reworks `WEEK_PLAN` from a fixed calendar into a function of declared gym-access days (see §2a's 3-layer design: saved default → weekly confirm-or-adjust → live same-day override), plus a guided/timed mock-AFT walkthrough as one of the session types the adaptive coach can pick on a gym day. Medium-large, no remaining open design questions, zero dependencies. Touches `src/core/training.js` (`WEEK_PLAN`), `src/tabs/plan.js` (`coach-today`, new gym-access UI), `src/tabs/aft.js`/`src/core/aft-scoring.js` (new guided-walkthrough UI over existing scoring math). **Test a few real gym-access patterns before shipping** (zero days, all seven, an irregular same-day change) and consider `npm run regress -- --shot` since the weekly render layout changes.
- Remaining phases after that: FM-2, X-Insight, T, FM-3, X-SmartFocus — see `IDEAS-tests-fm-workouts.md` for full scope/dependencies on each. **FM-3 (card-game workouts) is explicitly not build-greenlit even though its design is complete — Wyatt asked to hold it in planning; re-confirm before starting that one specifically, whenever its turn comes up.**
