...
3. `planning/IDEAS-tests-fm-workouts.md` — **the active workstream as of v186**, confirmed build order at the bottom. Read the "Decisions needed"/"Recommended build order" sections before picking the next phase.
...
**Current version: v186.** The service worker is at `operations-v186` in `sw.js`. `SKILL_LADDER_VER` is **118** (bumped in v186 for one guidance-text edit — see below). Total skills: **12524**. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)

**Read this before touching `src/tabs/test.js`'s `*Done()` functions:** every test's results screen used to be silently wiped in the same tick it was written — `stage.innerHTML=results; ...; render();` — because `render()` unconditionally rebuilds the whole Test tab as a side effect, including a blank stage div, immediately after. Fixed in v186 for all 8 tests by reordering to `render()` first, then re-query and write into the fresh post-render element. **If you add a 9th test (the next Phase T sub-phase), use this same order or its results screen will silently never show.**
...
## FM/test-features workstream — active as of v186, Phase T in progress (1/9 constructs shipped, 6 of 9 top-level phases fully done)
...
- **Phase T (stealth-assessment games) — IN PROGRESS, one construct shipped so far.** Per the doc's own sizing note, treated as N independent sub-phases (one construct at a time) rather than one big build. **Sentry (reaction time) shipped in v186** — night-watch game, threat silhouettes measured for real latency, decoys punish over-eager taps, real stats revealed only post-session. See its `FINISHED-FEATURES.md` entry, including a real render-wipe bug it surfaced and fixed across all 8 tests (read the warning above before touching `test.js` again). **Remaining constructs, all fully designed and confirmed in §1a, ready to build in any order:** Land Nav Relay (digit span), Comms Relay (typing), Perimeter Watch (n-back), Fire Discipline (go/no-go), Cipher Desk (processing speed), Fire Mission (mental math), Intel Briefing (reading), Climb the Tree (quizzes). Also still open from §1: the session-start "pick a focus" menu (present what-to-train-today instead of a flat test list) — not yet built, worth doing once more constructs are converted rather than redesigning the picker around a still-mostly-old-style list.
