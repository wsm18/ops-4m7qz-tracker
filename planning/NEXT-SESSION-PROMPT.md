...
**Current version: v209.** The service worker is at `operations-v209` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v209 touched no ladder/tier/guidance content). Total skills: **12524**.
...
**Queued — larger findings from the v209 audits that are design decisions, not simple bugs, none built yet:**
1. **The "remember things" skill-charter overlap** (`Memory span`/`Memory technique`/`Memory retention`/`Study & retention`) — still open since v202, not urgent.
2. **Skills tab's "⛓ Chain" view is confirmed fully redundant with the default Pyramid Tree browser** (stops at Rare tier, no actionable Combine buttons, doesn't lazy-load like the browser does) — the v209 audit recommends outright removal (`renderSynthesisChain()`, the button, its click handler, `skills.js`). Low-risk, hasn't been done yet since it's a feature removal worth a nod before deleting.
3. **Garden idol growth is now Path-size-biased.** Flat `+15` XP per skill-level-up (v201's fix) doesn't account for the real ~2x skill-count imbalance between Paths (~1565 skills in 6 Paths vs. ~783 in the other 4) — two users both "100% done" with their Path (by `catProgressFraction()`, the correct %) see very different Garden idol levels. Needs a decision: normalize the grant by path size, or reframe the Garden's copy as "cumulative activity" rather than "development" (it currently implies the latter).
4. **FM: no detraining/long-layoff awareness in `computeTarget()`.** The app's own copy (`plan.html`) explicitly promises "come back lighter" after a missed week+, but the adaptive engine has zero logic enforcing that — same progression after a 3-week break as continuous training. Rated HIGH value by the audit; the clear next FM priority if this keeps going.
5. **FM: no proactive PT-day scheduling coordination.** The app's intro copy promises PT-day awareness but only reacts *after* a PT session is logged (`recoveryLoad()`) — no pre-declared "these are my PT days" input feeding `assignWeekSessions()` the way gym-access already does.
6. **FM: `detectOvertrainingTrend()` (v205) only reads `S.workouts`, never `S.ptLog`** — a cadet training mostly via logged unit PT can never trip the deload advisory.
7. **Quiz: no timed/pressure-test mode** — real boards are largely verbal/rapid-fire; "Climb the Tree" is untimed multiple-choice. Rated the single biggest remaining board-readiness gap.
8. **Quiz: content gaps** — CBRN/NBC (zero coverage), TLP depth (only 2 of 8 steps tested despite `opords` covering the count), land nav depth (terrain-feature ID, pace count, resection/intersection not covered).
9. **Awards: inconsistent edit affordance** — Academic Honors/ROTC Record entries can only be added/deleted, not edited (typo = delete and retype).
10. **Weight tab: no cross-link to the Integrity skill it explicitly claims to mirror** (`skills-data.js` copy references the Weight ledger; neither tab links to the other).
11. **Profile: `S.donations` has zero skill/Path integration** — may be intentional scope, ask before building.

**Note if reading this in a fresh session with no conversation history:** the numbered list above is the full persisted detail — the original background-agent audit reports (5 in the v209 session, 3 in the v200 session, 2 in the v204 session) were never saved to a file beyond what's summarized here and in each version's `FINISHED-FEATURES.md` entry. If an item above needs more evidence than what's written, the right move is re-reading the relevant source file directly (each item names the file/function) or re-running a focused audit, not searching for a transcript that doesn't exist.

**A few small, low-risk findings from the original v200-session 3-agent audit were never explicitly closed out** (most of that audit's findings were fixed across v200-v203, but these two specific small ones don't appear in any version's changelog, so they're probably still open — confirm by reading the code before assuming):
- `src/tabs/profile.js`'s `spark()` function duplicates `miniSparkline()` (`src/core/state.js`) — a second hand-rolled sparkline implementation used only for the Vitals pulse/hemoglobin charts. Small, mechanical fix: delete `spark()`, call `miniSparkline(vals,90,22)` instead.
- `src/tabs/board.js`'s Board-task completion reward uses hardcoded literals (`grant(8,4,...)`) instead of the shared `VALUES` table (`constants.js`) other reward grants use, and the displayed reward text doesn't reflect the actual completion-bonus gold — medium-scope, multi-file if picked up.
