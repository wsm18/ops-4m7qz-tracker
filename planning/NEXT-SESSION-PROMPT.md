...
**Current version: v151.** The service worker is at `operations-v151` in `sw.js`. `SKILL_LADDER_VER` is currently **116** (in `src/core/migration.js`). Total skills: **3774**.

There is no `IMPROVEMENTS-vNNN.md` file right now. **Read the v148, v149, v150, and v151 entries in `FINISHED-FEATURES.md` before doing any pyramid-completeness auditing or Commons-layer work** — they document real mistakes from prior sessions:
...
- **Next: the Commons layer for the remaining 8 Mythic trees** (8,750 cards) — two trees down, one proven-working process, repeat it — see below

---

## What's next: continue writing Commons content, tree by tree

The prerequisite is done, the object shape is established, and two trees (Physical Mastery + Keeper of the Flame, 1250 skills) are written and verified as of v151. What's left is repeating that process for the other 8 Mythic trees (Tactical Mastery, Battlefield Commander, Cyberspace Operations Officer, Master of the Mind, Vital Operator, Scholar-Warrior, Sovereign Self, The Living Root — plus, eventually, the 6 second-generation Phase 7 trees):

1. **Confirm the current total** before starting, in case anything has drifted: `grep -oE 'synthesizedFrom:"[a-z0-9_]+_c_[a-z0-9_]+"' src/core/skills-data.js | sort -u | wc -l` should still read **2000** (one key per Uncommon, unchanged since v149 — Commons don't get their own `_c_` feeder keys, they're the floor). Total `SEED_SKILLS` should read **3774**. If either number is off, something changed and needs auditing the same way v148/v149/v150/v151 did.
2. **Pick the next Mythic tree** to complete (any of the 8 untouched ones — check for name+cat collision risk in that `cat` first via the duplicate sweep in the Ship checklist; `technical`/`personal`/`cognitive` carry the 10 known pre-existing collisions, so a first pass there needs extra care). Each tree is 125 setKeys / 625 skills, same shape as v150/v151.
3. **Reuse the v150/v151 Common object shape exactly** — no redesign needed: `rarity:"common"`, `fadeDays:30`, `setKey` = the parent Uncommon's own `synthesizedFrom` value, 4-item `levels`/`roadmap`/`advance`/`maintain` arrays, no `synthesizedFrom`/`unlockHint`/`tiers` fields. Grep any `phys_c_*` or `hearth_c_*` skill for a concrete reference.
4. Write `planning/IMPROVEMENTS-vNNN.md` scoping the chosen tree, then execute with the SLOT-marker + parallel-subagent pattern (see "Writing seeds at scale" below) — one agent per parent Rare (5 setKeys/25 skills, 25 agents total for a full tree). **Prefer smaller sequential waves (e.g. 4 agents per wave) over one giant dispatch** — v151 used this and had zero interruptions, versus v150's single 25-agent wave which got cut off mid-dispatch.
5. **Budget for a possible mid-dispatch interruption anyway.** Both v150 and a prior v151 attempt hit an account-wide session-limit cutoff partway through a dispatch. After ANY parallel dispatch — even one that reports full success — run `node --check` and a marker/member-count sweep FIRST, before trusting the agents' self-reported summaries, to catch a broken or partial state early. **This includes leftover/inherited content from a prior interrupted session** — v151 found 6 setKeys that were short a member (4/5) inside otherwise-valid leftover files from the interruption, which would have shipped short without counting members on inherited content too, not just newly-generated content. If interrupted, a small canary agent (one group) is a cheap way to confirm the limit has lifted before re-dispatching the rest.
6. After writing, run the full verification: syntax check, rarity-agnostic member-count sweep (every setKey has exactly 5 Common members), AND the name+cat duplicate sweep across the **whole file** — both v150 and v151 found new collisions this way (against sibling agents' output or against pre-existing content from earlier sessions), so this isn't optional even when every agent claims it pre-checked.
...
# bump sw.js: operations-v151 -> operations-v152 (or whatever's next)
...
**Writing seeds at scale (established v145, refined through v147):** pre-place unique `// SLOT:<setKey>:<name>:<cat>` marker comments yourself in one edit, then dispatch one subagent per marker group in parallel, **capped at 5 setKey groups (25 skills) per agent, never more** — each agent's edits persist to disk independently, so this cap bounds how much work a single failed or crashed agent call can take with it. Concurrent agents editing the same file will hit Edit-tool staleness retries — that's expected and safe, not a failure; each agent re-reads and retries or does an atomic script-based replace. Verify with the full build/check/regress pipeline yourself once at the end, not per-agent — and always run both the duplicate `name`+`cat` sweep and the rarity-agnostic member-count sweep too (see Ship checklist).
