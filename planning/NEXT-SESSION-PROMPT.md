...
**Current version: v203.** The service worker is at `operations-v203` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v203 touched no ladder/tier/guidance content). Total skills: **12524**. **v199: the Today tab's compact session preview (`dawnSessionHtml()`, plan.js) now shows real per-exercise targets via the same `computeTarget()` call Coach Today makes — was names-only before.** This closes the "make everything use coach" ask from v197/v198. **v200 fixed 2 small live bugs** (Board's `grant()` used an invalid `"knowledge"` path key instead of `"academic"`; Battle Buddy Report read `a.n`/`S.events.hours` instead of the real `a.title`/`S.volunteer`). **v201 fixed the broken Path-development math** (see the v201 `FINISHED-FEATURES.md` entry — `catRolledLevel()` deleted entirely, `catProgressFraction()` as a % is now the one canonical Path-development number everywhere; Garden's idol growth now credits the leveled skill's own Path instead of always Academic) **and found + fixed the real root cause of the long-flagged v195 "cosmetic NaN" warning** (a copy-paste seed-hash bitmask typo in `skEmblemSvg`'s `_physical()` sigil generator, `skills.js`). **v202 fixed the 2 permanently-stuck-at-0 skills** — built a real 10th stealth-assessment game (Signal Intercept, `test.js`) for Pattern recognition, and wired Memory retention to real SRS ease-factor stats (`auto-level.js`'s new `memRetentionLevel()`). **v203 consolidated the 5 competing "what skill needs attention" systems on Dawn** — `getWarriorsFocus()` and the "Skill of the day" row both now read `computeSmartFocus()` instead of their own independent urgency logic, with duplicate-suppression when they'd show the same skill twice.

**All 3 FM-redesign-sized workstreams found by the v200-session 3-agent audit are now shipped (v201/v202/v203) — no queued workstream remains.** The next session should open by asking Wyatt what he wants to work on next.
