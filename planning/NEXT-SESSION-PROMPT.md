...
**Current version: v201.** The service worker is at `operations-v201` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v201 touched no ladder/tier/guidance content). Total skills: **12524**. **v199: the Today tab's compact session preview (`dawnSessionHtml()`, plan.js) now shows real per-exercise targets via the same `computeTarget()` call Coach Today makes — was names-only before.** This closes the "make everything use coach" ask from v197/v198. **v200 fixed 2 small live bugs** (Board's `grant()` used an invalid `"knowledge"` path key instead of `"academic"`; Battle Buddy Report read `a.n`/`S.events.hours` instead of the real `a.title`/`S.volunteer`). **v201 fixed the broken Path-development math** (see the v201 `FINISHED-FEATURES.md` entry — `catRolledLevel()` deleted entirely, `catProgressFraction()` as a % is now the one canonical Path-development number everywhere; Garden's idol growth now credits the leveled skill's own Path instead of always Academic) **and found + fixed the real root cause of the long-flagged v195 "cosmetic NaN" warning** (a copy-paste seed-hash bitmask typo in `skEmblemSvg`'s `_physical()` sigil generator, `skills.js`).

**Two more FM-redesign-sized workstreams are still queued, found by the same v200-session 3-agent audit and confirmed wanted by Wyatt ("please do all of these") — read the v200/v201 `FINISHED-FEATURES.md` entries before starting either:**
1. **Two skills are permanently stuck at level 0.** `Pattern recognition` (`auto:"test:patterns"`) and `Memory retention` (`auto:"quiz:retention"`) are locked as "auto-leveling" but nothing in `auto-level.js`/`test.js` actually levels either — no `"patterns"` test exists, no `"quiz:retention"` handler exists. `Memory retention`'s own `howTo` copy directly contradicts its lock ("there is no automatic leveling for this skill yet — honesty here is the whole point"). Broader: the "remember things" concept is split across 4 skills (`Memory span`, `Memory technique`, `Memory retention`, `Study & retention`) with no shared model. **Wyatt confirmed via `AskUserQuestion`:** build a 10th Phase-T-style stealth-assessment game for Pattern recognition (matching Sentry/Land Nav Relay/etc. — hide the score during play, honest results after), and wire `Memory retention` to real SRS stats from `S.srsDecks` (ease/interval/reps) instead of leaving it dead. Still needs its own design pass for the game itself (confirm the concrete game mechanic with Wyatt before building, same as Phase T's original process) and for reconciling the 4-skill cluster's charters so they measure genuinely distinct things.
2. **Five competing "what skill needs attention" systems on the Today/Dawn tab** (`getWarriorsFocus()`'s decay step, Today's Hand random draw, `computeSmartFocus()`'s callout, the "Skill of the day" block, and the "Fading soon" digest) can each recommend a different skill simultaneously, computed via 3 different independent urgency thresholds. `computeSmartFocus()` is the newest/most deliberate and should likely become the one canonical ranking, with Today's Hand kept as the one intentionally-different "practice, not urgency" mode.
