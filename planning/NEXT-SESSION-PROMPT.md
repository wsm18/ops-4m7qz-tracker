...
**Current version: v199.** The service worker is at `operations-v199` in `sw.js`. `SKILL_LADDER_VER` is **118** (unchanged since v186 — v187 through v199 touched no ladder/tier/guidance content). Total skills: **12524**. **v199: the Today tab's compact session preview (`dawnSessionHtml()`, plan.js) now shows real per-exercise targets via the same `computeTarget()` call Coach Today makes — was names-only before.** This closes the "make everything use coach" ask from v197/v198. (v182-v184 were quick interrupts, not FM-doc phases — every subjective self-rating input in the app (effort, readiness, PT intensity, session RPE) now uses a 1-10 scale; this is now a **standing convention in `CLAUDE.md`** for any future rating input. See the v182/v183/v184 entries in `FINISHED-FEATURES.md`.)
