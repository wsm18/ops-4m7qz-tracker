...
**Explicitly still open (Phase B, not scoped further yet):** migrating the other ~15 tabs' bespoke button/card classes onto the new shared system; revisiting each tab's per-tab "visual atmosphere" treatment now that a real base exists; deciding intentionally whether the Weight tab's serif font + separate tan palette (looks like one deliberate "set apart" choice) is a keeper; any deeper interaction change to the More/mobile drawer beyond static group labels (e.g. collapsible sections).
