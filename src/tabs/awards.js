...
document.body.addEventListener("click",e=>{
  ...
  const avoidTagBtn=e.target.closest("[data-avoidtag]"); if(avoidTagBtn){ const k=avoidTagBtn.dataset.avoidtag; if(!S.avoidTags) S.avoidTags=[]; const i=S.avoidTags.indexOf(k); if(i<0) S.avoidTags.push(k); else S.avoidTags.splice(i,1); save(); render(); return; }
